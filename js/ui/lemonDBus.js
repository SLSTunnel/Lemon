...
const LemonIface =
    '<node> \
        <interface name="org.Lemon"> \
            ...
            <property name="LemonVersion" type="s" access="read" /> \
...
LemonDBus.prototype = {
    _init: function() {
        this._dbusImpl = Gio.DBusExportedObject.wrapJSObject(LemonIface, this);
    ...
    updateSetting: function(uuid, instance_id, key, payload) {
        if (!Main.settingsManager.uuids[uuid]) {
            global.logWarning(
                `[LemonDBus] [${uuid}] Unable to find UUID from SettingsManager - ` +
    ...
    RestartLemon: function(showOsd) {
        Main.restartLemon(showOsd);
    },
    ...
    LemonVersion: Config.PACKAGE_VERSION
};
