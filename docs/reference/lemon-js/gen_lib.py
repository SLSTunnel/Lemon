...
def get_type_link(typ, file):
    ...
    if typ == '':
        ...
        if typ in objects:
            return "lemon-js-" + objects[typ].prefix
        elif file.name + "." + typ in objects:
            return "lemon-js-" + objects[file.name + "." + typ].prefix
        elif typ.endswith("s") and typ[:-1] in objects:
            return "lemon-js-" + objects[typ[:-1]].prefix
        elif typ.endswith("s") and file.name + "." + typ[:-1] in objects:
            return "lemon-js-" + objects[file.name + "." + typ[:-1]].prefix
...
def markup(line, obj):
    ...
    def format_ext_link(match):
        ...
        if thing in prop_names and not full.endswith("()"):
            return '<link linkend="lemon-js-{prefix}--{thing}"><code>{full}</code></link>'.format(
...
SGML_CHAPTER_FORMAT = '''
<chapter id="lemon-js-{prefix}-section">
  <title>{title}</title>
  {entries}
</chapter>'''
...
<refentry id="lemon-js-{prefix}">
  <refmeta>
    <refentrytitle role="top_of_page" id="lemon-js-{prefix}.top_of_page">{name}</refentrytitle>
...
<refsect1 id="lemon-js-{prefix}.functions" role="functions_proto">
...
<row>
  ...
  <entry role="function_name">
    <link linkend="lemon-js-{prefix}-{name}">{name}</link>&#160;<phrase role="c_punctuation">()</phrase>
  </entry>
</row>
'''

PROPERTY_HEADER_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.properties" role="properties">
...
<refsect1 id="lemon-js-{prefix}.signals" role="signal_proto">
...
<row>
  ...
  <entry role="signal_name">
    <link linkend="lemon-js-{prefix}-{name}-signal">{name}</link>
  </entry>
</row>
'''

ENUM_HEADER_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.other" role="other_proto">
...
<row>
  ...
  <entry role="function_name">
    <link linkend="lemon-js-{prefix}-{name}">{name}</link>
  </entry>
</row>
'''
...
HIERARCHY_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.object-hierarchy" role="object_hierarchy">
...
HIERARCHY_ITEM_FORMAT = '{spacing}<phrase role="lineart">&#9584;&#9472;&#9472;</phrase> <link linkend="lemon-js-{prefix}">{name}</link>'

DESCRIPTION_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.description" role="desc">
...
FUNCTIONS_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.functions_details" role="details">
...
<refsect2 id="lemon-js-{prefix}-{name}" role="function">
  <title>{name}&#160;()</title>
  <indexterm zone="lemon-js-{prefix}-{name}"><primary>{name}</primary></indexterm>
...
SIGNALS_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.signal-details" role="details">
...
SIGNAL_ITEM_FORMAT = '''
<refsect2 id="lemon-js-{prefix}-{name}-signal" role="signal">
  <title>The <literal>“{name}”</literal> signal</title>
  <indexterm zone="lemon-js-{prefix}-{name}-signal"><primary>{prefix}::{name}</primary></indexterm>
  <programlisting language="javascript">
...
PROPERTIES_FORMAT = '''
<refsect1 id="lemon-js-{prefix}.property-details" role="property_details">
...
<refsect2 id="lemon-js-{prefix}--{name}" role="property">
  <title>The <literal>“{name}”</literal> property</title>
  <indexterm zone="lemon-js-{prefix}--{name}">
    <primary>lemon-js-{prefix}:{name}</primary>
  </indexterm>
...
ENUMS_ITEM_FORMAT = '''
<refsect2 id="lemon-js-{prefix}" role="enum">
