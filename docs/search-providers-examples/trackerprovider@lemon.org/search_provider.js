            ...
            for (var i in results)
            {
                switch (results[i].type)
                {
                    ...
                    case "software":
                        ...
                        var app = Lemon.AppSystem.get_default().lookup_app(results_parts[results_parts.length - 1].split(".desktop")[0] + ".desktop");
                        if (!app)
                        {
                            continue;
                        }
...
function perform_search(pattern)
{
    ...
    Util.spawn_async(['python', SearchProviderManager.extensionMeta['trackerprovider@lemon.org'].path + '/search_provider.py', pattern], Lang.bind(this, dbus_push_results, pattern));
}
