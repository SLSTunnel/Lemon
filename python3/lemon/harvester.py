...
SPICE_MAP = {
    ...
    "theme": {
        ...
        "enabled-schema": "org.lemon.theme",
        "enabled-key": "name"
    }
}
...
class SpicePathSet:
    def __init__(self, cache_item, spice_type):
        cache_folder = Path(os.path.join(GLib.get_user_cache_dir(), 'lemon', 'spices', spice_type))
...
class Harvester:
    def __init__(self, spice_type):
        ...
        self.cache_folder = os.path.join(GLib.get_user_cache_dir(), 'lemon', 'spices', self.spice_type)

        self.index_file = os.path.join(self.cache_folder, "index.json")

        self.install_folder = f"{home}/.local/share/nemo/actions" if self.actions else os.path.join(home, ".local/share/lemon", f"{self.spice_type}s")
