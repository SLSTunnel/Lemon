...
struct _LemonApp
{
  ...
  GMenuTreeEntry *entry; /* If NULL, this app is backed by one or more
                          ...
                          * the way lemon-window-tracker.c works).
                          */
  GMenuDesktopAppInfo *info;

  LemonAppRunningState *running_state;
...
G_DEFINE_TYPE (LemonApp, lemon_app, G_TYPE_OBJECT);
...
static guint lemon_app_signals[LAST_SIGNAL] = { 0 };

static void create_running_state (LemonApp *app);
static void unref_running_state (LemonAppRunningState *state);

static void
lemon_app_get_property (GObject    *gobject,
                        guint       prop_id,
                        GValue     *value,
                        GParamSpec *pspec)
{
  LemonApp *app = LEMON_APP (gobject);
...
char *
lemon_app_get_flatpak_app_id (LemonApp *app)
{
  if (app->info)
  {
    gchar *id;
    ...
    {
        const gchar *desktop_file = lemon_app_get_id (app);
...
static MetaWindow *
window_backed_app_get_window (LemonApp     *app)
{
...
static ClutterActor *
get_actor_for_icon_name (LemonApp *app,
...
static ClutterActor *
window_backed_app_get_icon (LemonApp *app,
                            int       size)
{
  ...
  global = lemon_global_get ();
  context = st_theme_context_get_for_stage (lemon_global_get_stage (global));
 ...
 */
ClutterActor *
lemon_app_create_icon_texture (LemonApp   *app,
                               int         size)
{
  GIcon *icon;
  ClutterActor *ret;
...
/**
 * lemon_app_create_icon_texture_for_window:
 * @app: a #LemonApp
 ...
 */
ClutterActor *
lemon_app_create_icon_texture_for_window (LemonApp   *app,
                                             int            size,
                                             MetaWindow    *for_window)
{
    ...
    if (for_window != NULL)
      {
        if (g_slist_find (app->running_state->windows, for_window) != NULL)
          ...
          {
            g_warning ("lemon_app_create_icon_texture: MetaWindow %p provided that does not match App %p",
                       for_window, app);
          }
      }
  ...
  return lemon_app_create_icon_texture (app, size);
}

static const char *
get_common_name (LemonApp *app)
{
  if (app->entry)
...
const char *
lemon_app_get_name (LemonApp *app)
{
  if (app->unique_name)
...
const char *
lemon_app_get_description (LemonApp *app)
{
  if (app->entry)
...
const char *
lemon_app_get_keywords (LemonApp *app)
{
...
gboolean
lemon_app_get_nodisplay (LemonApp *app)
{
 ...
 */
gboolean
lemon_app_is_window_backed (LemonApp *app)
{
 ...
 */
void
lemon_app_activate_window (LemonApp     *app,
                           MetaWindow   *window,
                           guint32       timestamp)
{
  GSList *windows;

  if (app->state != LEMON_APP_STATE_RUNNING)
    return;

  windows = lemon_app_get_windows (app);
 ...
 * @app: a #LemonApp
 *
 * Like lemon_app_activate_full(), but using the default workspace and
 * event timestamp.
 */
void
lemon_app_activate (LemonApp      *app)
{
  return lemon_app_activate_full (app, -1, 0);
}

/**
 * lemon_app_activate_full:
 * @app: a #LemonApp
 ...
 */
void
lemon_app_activate_full (LemonApp      *app,
                         int            workspace,
                         guint32        timestamp)
{
  LemonGlobal *global;
  ...
  if (timestamp == 0)
    timestamp = lemon_global_get_current_time (global);
  ...
  switch (app->state)
    {
      case LEMON_APP_STATE_STOPPED:
        {
          ...
          if (!lemon_app_launch (app,
            ...
            {
              char *msg;
              msg = g_strdup_printf (_("Failed to launch '%s'"), lemon_app_get_name (app));
              lemon_global_notify_error (global,
                                         msg,
                                         error->message);
              g_free (msg);
              g_clear_error (&error);
      ...
      case LEMON_APP_STATE_RUNNING:
        lemon_app_activate_window (app, NULL, timestamp);
        break;
      default:
        g_warning("lemon_app_activate_full: default case");
        break;
    }
}

/**
 * lemon_app_open_new_window:
 * @app: a #LemonApp
 ...
 */
void
lemon_app_open_new_window (LemonApp      *app,
                           int            workspace)
{
 ...
 */
LemonAppState
lemon_app_get_state (LemonApp *app)
{
  return app->state;
}

/**
 * lemon_app_get_is_flatpak:
 * @app: a #LemonApp
 *
 * Returns: TRUE if #app is a flatpak app, FALSE if not
 */
gboolean
lemon_app_get_is_flatpak (LemonApp *app)
{
...
static int
lemon_app_compare_windows (gconstpointer   a,
                           gconstpointer   b,
                           gpointer        datap)
{
 ...
 */
GSList *
lemon_app_get_windows (LemonApp *app)
{
  if (app->running_state == NULL)
    return NULL;
  ...
  if (app->running_state->window_sort_stale)
    {
      ...
      app->running_state->windows = g_slist_sort_with_data (app->running_state->windows, lemon_app_compare_windows, &data);
...
guint
lemon_app_get_n_windows (LemonApp *app)
{
  if (app->running_state == NULL)
    return 0;
...
gboolean
lemon_app_is_on_workspace (LemonApp *app,
                           MetaWorkspace   *workspace)
{
  GSList *iter;

  if (app->state == LEMON_APP_STATE_STARTING)
    {
...
LemonApp *
_lemon_app_new_for_window (MetaWindow      *window)
{
  LemonApp *app;

  app = g_object_new (LEMON_TYPE_APP, NULL);

  app->window_id_string = g_strdup_printf ("window:%d", meta_window_get_stable_sequence (window));

  _lemon_app_add_window (app, window);

  return app;
}

LemonApp *
_lemon_app_new (GMenuTreeEntry *info)
{
  LemonApp *app;

  app = g_object_new (LEMON_TYPE_APP, NULL);

  _lemon_app_set_entry (app, info);

  return app;
}

void
_lemon_app_set_entry (LemonApp       *app,
...
static void
lemon_app_state_transition (LemonApp      *app,
                            LemonAppState  state)
{
  if (app->state == state)
    return;
  g_return_if_fail (!(app->state == LEMON_APP_STATE_RUNNING &&
                      state == LEMON_APP_STATE_STARTING));
  app->state = state;

  if (app->state == LEMON_APP_STATE_STOPPED && app->running_state)
    {
      unref_running_state (app->running_state);
      app->running_state = NULL;
    }

  _lemon_app_system_notify_app_state_changed (lemon_app_system_get_default (), app);

  g_object_notify (G_OBJECT (app), "state");
}

static void
lemon_app_on_unmanaged (MetaWindow      *window,
                        LemonApp *app)
{
  _lemon_app_remove_window (app, window);
}

static void
lemon_app_on_ws_switch (MetaWorkspaceManager *workspace_manager,
                        int                 from,
                        int                 to,
                        MetaMotionDirection direction,
                        gpointer            data)
{
  LemonApp *app = LEMON_APP (data);

  g_assert (app->running_state != NULL);

  app->running_state->window_sort_stale = TRUE;

  g_signal_emit (app, lemon_app_signals[WINDOWS_CHANGED], 0);
}

void
_lemon_app_add_window (LemonApp        *app,
                       MetaWindow      *window)
{
  ...
  g_signal_connect (window, "unmanaged", G_CALLBACK(lemon_app_on_unmanaged), app);

  if (app->state != LEMON_APP_STATE_STARTING)
    lemon_app_state_transition (app, LEMON_APP_STATE_RUNNING);
...
void
_lemon_app_remove_window (LemonApp   *app,
                          MetaWindow *window)
{
  g_assert (app->running_state != NULL);
  ...
  g_signal_handlers_disconnect_by_func (window, G_CALLBACK(lemon_app_on_unmanaged), app);
  g_object_unref (window);
  app->running_state->windows = g_slist_remove (app->running_state->windows, window);

  if (app->running_state->windows == NULL)
    lemon_app_state_transition (app, LEMON_APP_STATE_STOPPED);
 ...
 */
GSList *
lemon_app_get_pids (LemonApp *app)
{
  GSList *result;
  GSList *iter;

  result = NULL;
  for (iter = lemon_app_get_windows (app); iter; iter = iter->next)
    {
      MetaWindow *window = iter->data;
...
void
_lemon_app_handle_startup_sequence (LemonApp          *app,
                                       MetaStartupSequence  *sequence)
{
   ...
   */
  if (starting && lemon_app_get_state (app) == LEMON_APP_STATE_STOPPED)
    {
      MetaDisplay *display = lemon_global_get_display (lemon_global_get ());

      lemon_app_state_transition (app, LEMON_APP_STATE_STARTING);
...
const char *
_lemon_app_get_common_name (LemonApp *app)
{
  return get_common_name (app);
}

void
_lemon_app_set_unique_name (LemonApp *app,
                               gchar       *unique_name)
{
  if (app->unique_name)
    {
...
const char *
_lemon_app_get_unique_name (LemonApp *app)
{
  return app->unique_name;
}

const char *
_lemon_app_get_executable (LemonApp *app)
{
  if (app->entry)
    {
...
const char *
_lemon_app_get_desktop_path (LemonApp *app)
{
  if (app->entry)
    {
...
void
_lemon_app_set_hidden_as_duplicate (LemonApp *app,
                                     gboolean     hide)
{
 ...
 */
gboolean
lemon_app_request_quit (LemonApp   *app)
{
  LemonGlobal *global;
  GSList *iter;
  ...
  for (iter = app->running_state->windows; iter; iter = iter->next)
    {
      ...
      meta_window_delete (win, lemon_global_get_current_time (global));
    }
  return TRUE;
}
...
{
  LemonApp *app;
  LemonWindowTracker *tracker;

  g_return_if_fail (data != NULL);

  app = LEMON_APP (data);
  tracker = lemon_window_tracker_get_default ();

  _lemon_window_tracker_add_child_process_app (tracker,
                                               pid,
                                               app);
}
                  ...
                  GError      **error)
{
  ...
  context = lemon_global_create_app_launch_context (global);
  ...
  if (offload)
    {
      ...
      g_debug ("Offloading '%s' to discrete gpu.", lemon_app_get_name (app));
...
lemon_app_launch (LemonApp     *app,
                     ...
                     GError         **error)
{
  GMenuDesktopAppInfo *app_info = lemon_app_get_app_info(app);
 ...
 */
gboolean
lemon_app_launch_offloaded (LemonApp     *app,
                               guint            timestamp,
                               GList           *uris,
                               int              workspace,
                               char           **startup_id,
...
create_running_state (LemonApp *app)
{
  ...
  app->running_state = g_slice_new0 (LemonAppRunningState);
  app->running_state->refcount = 1;
  app->running_state->workspace_switch_id =
    g_signal_connect (workspace_manager, "workspace-switched", G_CALLBACK(lemon_app_on_ws_switch), app);
}

static void
unref_running_state (LemonAppRunningState *state)
{
  MetaWorkspaceManager *workspace_manager;
  LemonGlobal *global;
  ...
  g_slice_free (LemonAppRunningState, state);
}

static void
lemon_app_init (LemonApp *self)
{
  self->state = LEMON_APP_STATE_STOPPED;
  self->keywords = NULL;
  self->global = lemon_global_get ();
}

static void
lemon_app_dispose (GObject *object)
{
  LemonApp *app = LEMON_APP (object);
  ...
  while (app->running_state)
    _lemon_app_remove_window (app, app->running_state->windows->data);

  g_clear_pointer (&app->keywords, g_free);
  g_clear_pointer (&app->unique_name, g_free);

  G_OBJECT_CLASS(lemon_app_parent_class)->dispose (object);
}

static void
lemon_app_finalize (GObject *object)
{
  LemonApp *app = LEMON_APP (object);

  g_free (app->window_id_string);

  G_OBJECT_CLASS(lemon_app_parent_class)->finalize (object);
}

static void
lemon_app_class_init(LemonAppClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = lemon_app_get_property;
  gobject_class->dispose = lemon_app_dispose;
  gobject_class->finalize = lemon_app_finalize;

  lemon_app_signals[WINDOWS_CHANGED] = g_signal_new ("windows-changed",
                                     LEMON_TYPE_APP,
                                     G_SIGNAL_RUN_LAST,
                                     0,
                                     NULL, NULL, NULL,
