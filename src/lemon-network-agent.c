...
struct _LemonNetworkAgentPrivate {
  /* <gchar *request_id, LemonAgentRequest *request> */
  GHashTable *requests;
};

G_DEFINE_TYPE_WITH_PRIVATE (LemonNetworkAgent, lemon_network_agent, NM_TYPE_SECRET_AGENT_OLD)

static const SecretSchema network_agent_schema = {
    "org.freedesktop.NetworkManager.Connection",
    SECRET_SCHEMA_DONT_MATCH_NAME,
    {
        { LEMON_KEYRING_UUID_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { LEMON_KEYRING_SN_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { LEMON_KEYRING_SK_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { NULL, 0 },
    }
};

static void
lemon_agent_request_free (gpointer data)
{
  LemonAgentRequest *request = data;
...
static void
lemon_agent_request_cancel (LemonAgentRequest *request)
{
  GError *error;
  LemonNetworkAgent *self;
...
static void
lemon_network_agent_init (LemonNetworkAgent *agent)
{
  LemonNetworkAgentPrivate *priv;

  priv = agent->priv = lemon_network_agent_get_instance_private (agent);
  priv->requests = g_hash_table_new_full (g_str_hash, g_str_equal,
                      g_free, lemon_agent_request_free);
}

static void
lemon_network_agent_finalize (GObject *object)
{
  LemonNetworkAgentPrivate *priv = LEMON_NETWORK_AGENT (object)->priv;
  GError *error;
  GHashTableIter iter;
  gpointer key;
  gpointer value;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      LemonAgentRequest *request = value;
  ...
  G_OBJECT_CLASS (lemon_network_agent_parent_class)->finalize (object);
}

static void
request_secrets_from_ui (LemonAgentRequest *request)
{
  ...
  for (l = items; l; l = g_list_next (l))
    {
      ...
      while (g_hash_table_iter_next (&iter, (gpointer *)&name, (gpointer *)&attribute))
        {
          if (g_strcmp0 (name, LEMON_KEYRING_SK_TAG) == 0)
            {
...
static void
lemon_network_agent_get_secrets (NMSecretAgentOld                 *agent,
                                    NMConnection                     *connection,
                                    ...
                                    gpointer                          callback_data)
{
  LemonNetworkAgent *self = LEMON_NETWORK_AGENT (agent);
  LemonAgentRequest *request;
  GHashTable *attributes;
  char *request_id;
  ...
  if ((request = g_hash_table_lookup (self->priv->requests, request_id)) != NULL)
    {
       ...
       */
      lemon_agent_request_cancel (request);
    }

  request = g_new0 (LemonAgentRequest, 1);
  ...
  attributes = secret_attributes_build (&network_agent_schema,
                                        LEMON_KEYRING_UUID_TAG, nm_connection_get_uuid (connection),
                                        LEMON_KEYRING_SN_TAG, setting_name,
                                        NULL);
...
void
lemon_network_agent_add_vpn_secret (LemonNetworkAgent *self,
                                       gchar                *request_id,
                                       gchar                *setting_key,
                                       gchar                *setting_value)
{
  ...
  g_return_if_fail (LEMON_IS_NETWORK_AGENT (self));
...
void
lemon_network_agent_set_password (LemonNetworkAgent *self,
                                     gchar                *request_id,
                                     gchar                *setting_key,
                                     gchar                *setting_value)
{
...
void
lemon_network_agent_respond (LemonNetworkAgent         *self,
                                gchar                        *request_id,
                                LemonNetworkAgentResponse  response)
{
  ...
  if (response == LEMON_NETWORK_AGENT_USER_CANCELED)
    {
  ...
  if (response == LEMON_NETWORK_AGENT_INTERNAL_ERROR)
    {
      GError *error = g_error_new (NM_SECRET_AGENT_ERROR,
                                   NM_SECRET_AGENT_ERROR_FAILED,
  ...
  /* response == LEMON_NETWORK_AGENT_CONFIRMED */
...
void
lemon_network_agent_search_vpn_plugin (LemonNetworkAgent   *self,
                                          const char          *service,
                                          ...
                                          gpointer             user_data)
{
  ...
  g_task_set_source_tag (task, lemon_network_agent_search_vpn_plugin);
...
/**
 * lemon_network_agent_search_vpn_plugin_finish:
 *
 * Returns: (nullable) (transfer full): The found plugin or %NULL
 */
NMVpnPluginInfo *
lemon_network_agent_search_vpn_plugin_finish (LemonNetworkAgent  *self,
                                                 GAsyncResult       *result,
                                                 GError            **error)
{
  g_return_val_if_fail (LEMON_IS_NETWORK_AGENT (self), NULL);
...
static void
lemon_network_agent_cancel_get_secrets (NMSecretAgentOld *agent,
                                           const gchar      *connection_path,
                                           const gchar      *setting_name)
{
  LemonNetworkAgent *self = LEMON_NETWORK_AGENT (agent);
  LemonNetworkAgentPrivate *priv = self->priv;
  gchar *request_id;
  LemonAgentRequest *request;
                              ...
                              gchar       **out_display_name)
{
  ...
  return secret_attributes_build (&network_agent_schema,
                                  LEMON_KEYRING_UUID_TAG, connection_uuid,
                                  LEMON_KEYRING_SN_TAG, setting_name,
                                  LEMON_KEYRING_SK_TAG, setting_key,
                                  NULL);
}
...
static void
lemon_network_agent_save_secrets (NMSecretAgentOld                *agent,
                                     NMConnection                    *connection,
                                     const gchar                     *connection_path,
...
static void
lemon_network_agent_delete_secrets (NMSecretAgentOld                  *agent,
                                       NMConnection                      *connection,
                                       const gchar                       *connection_path,
                                       ...
                                       gpointer                           callback_data)
{
  ...
  secret_password_clear (&network_agent_schema, NULL, delete_items_cb, r,
                         LEMON_KEYRING_UUID_TAG, uuid,
                         NULL);
}

void
lemon_network_agent_class_init (LemonNetworkAgentClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  NMSecretAgentOldClass *agent_class = NM_SECRET_AGENT_OLD_CLASS (klass);

  gobject_class->finalize = lemon_network_agent_finalize;

  agent_class->get_secrets = lemon_network_agent_get_secrets;
  agent_class->cancel_get_secrets = lemon_network_agent_cancel_get_secrets;
  agent_class->save_secrets = lemon_network_agent_save_secrets;
  agent_class->delete_secrets = lemon_network_agent_delete_secrets;
