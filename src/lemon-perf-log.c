...
typedef struct _LemonPerfEvent LemonPerfEvent;
typedef struct _LemonPerfStatistic LemonPerfStatistic;
typedef struct _LemonPerfStatisticsClosure LemonPerfStatisticsClosure;
typedef union  _LemonPerfStatisticValue LemonPerfStatisticValue;
typedef struct _LemonPerfBlock LemonPerfBlock;

/**
 * SECTION:lemon-perf-log
 * @short_description: Event recorder for performance measurement
 *
 * LemonPerfLog provides a way for different parts of the code to
...
struct _LemonPerfLogClass
{
  GObjectClass parent_class;
};

struct _LemonPerfEvent
{
  guint16 id;
  char *name;
  char *description;
...
union _LemonPerfStatisticValue
{
  int i;
  gint64 x;
};

struct _LemonPerfStatistic
{
  LemonPerfEvent *event;

  LemonPerfStatisticValue current_value;
  LemonPerfStatisticValue last_value;

  guint initialized : 1;
  guint recorded : 1;
};

struct _LemonPerfStatisticsClosure
{
...
G_DEFINE_TYPE(LemonPerfLog, lemon_perf_log, G_TYPE_OBJECT);
...
static void
lemon_perf_log_init (LemonPerfLog *perf_log)
{
   ...
   * logging is enabled some time after starting Lemon */
  lemon_perf_log_define_event (perf_log, "perf.setTime", "", "x");
   ...
   */
  lemon_perf_log_define_event (perf_log, "perf.statisticsCollected",
...
static void
lemon_perf_log_class_init (LemonPerfLogClass *class)
{
}

/**
 * lemon_perf_log_get_default:
 *
 * Gets the global singleton performance log. This is initially disabled
 * and must be explicitly enabled with lemon_perf_log_set_enabled().
 *
 * Return value: (transfer none): the global singleton performance log
 */
LemonPerfLog *
lemon_perf_log_get_default (void)
{
  static LemonPerfLog *perf_log;

  if (perf_log == NULL)
    perf_log = g_object_new (LEMON_TYPE_PERF_LOG, NULL);
...
{
  LemonPerfLog *perf_log = data;

  lemon_perf_log_collect_statistics (perf_log);

  return TRUE;
}

/**
 * lemon_perf_log_set_enabled:
 * @perf_log: a #LemonPerfLog
 * @enabled: whether to record events
 *
 * Sets whether events are currently being recorded.
 */
void
lemon_perf_log_set_enabled (LemonPerfLog *perf_log,
                            gboolean      enabled)
{
...
static LemonPerfEvent *
define_event (LemonPerfLog *perf_log,
              const char   *name,
              ...
              const char   *signature)
{
  ...
  event = g_slice_new (LemonPerfEvent);
 ...
 */
void
lemon_perf_log_define_event (LemonPerfLog *perf_log,
                             const char   *name,
                             const char   *description,
                             const char   *signature)
{
...
static LemonPerfEvent *
lookup_event (LemonPerfLog *perf_log,
              const char   *name,
              const char   *signature)
{
  LemonPerfEvent *event = g_hash_table_lookup (perf_log->events_by_name, name);
...
static void
record_event (LemonPerfLog   *perf_log,
              gint64          event_time,
              LemonPerfEvent *event,
              const guchar   *bytes,
              size_t          bytes_len)
{
  LemonPerfBlock *block;
  size_t total_bytes;
  guint32 time_delta;
  guint32 pos;
  ...
  if (perf_log->blocks->tail == NULL ||
      total_bytes + ((LemonPerfBlock *)perf_log->blocks->tail->data)->bytes > BLOCK_SIZE)
    {
      block = g_new (LemonPerfBlock, 1);
      block->bytes = 0;
      g_queue_push_tail (perf_log->blocks, block);
    }
  else
    {
      block = (LemonPerfBlock *)perf_log->blocks->tail->data;
    }
 ...
 */
void
lemon_perf_log_event (LemonPerfLog *perf_log,
                      const char   *name)
{
  LemonPerfEvent *event = lookup_event (perf_log, name, "");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
lemon_perf_log_event_i (LemonPerfLog *perf_log,
                        const char   *name,
                        gint32        arg)
{
  LemonPerfEvent *event = lookup_event (perf_log, name, "i");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
lemon_perf_log_event_x (LemonPerfLog *perf_log,
                        const char   *name,
                        gint64        arg)
{
  LemonPerfEvent *event = lookup_event (perf_log, name, "x");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
lemon_perf_log_event_s (LemonPerfLog *perf_log,
                         const char   *name,
                         const char   *arg)
{
  LemonPerfEvent *event = lookup_event (perf_log, name, "s");
  if (G_UNLIKELY (event == NULL))
    return;
...
/**
 * lemon_perf_log_define_statistic:
 * @perf_log: a #LemonPerfLog
 * @name: name of the statistic and of the corresponding event.
 *  This should follow the same guidelines as for lemon_perf_log_define_event()
 ...
 * lemon_perf_log_collect_statistics() is called explicitly.
 *
 ...
 * such as lemon_perf_log_update_statistic_i(). This can be done
 * at any time, but would normally done inside a function registered
 * with lemon_perf_log_add_statistics_callback(). These functions
 * are called immediately before statistics are recorded.
 */
void
lemon_perf_log_define_statistic (LemonPerfLog *perf_log,
                                 const char   *name,
                                 const char   *description,
                                 const char   *signature)
{
  LemonPerfEvent *event;
  LemonPerfStatistic *statistic;
  ...
  statistic = g_slice_new (LemonPerfStatistic);
...
static LemonPerfStatistic *
lookup_statistic (LemonPerfLog *perf_log,
                  const char   *name,
                  const char   *signature)
{
  LemonPerfStatistic *statistic = g_hash_table_lookup (perf_log->statistics_by_name, name);
...
/**
 * lemon_perf_log_update_statistic_i:
 * @perf_log: a #LemonPerfLog
 ...
 */
void
lemon_perf_log_update_statistic_i (LemonPerfLog *perf_log,
                                   const char   *name,
                                   gint32        value)
{
  LemonPerfStatistic *statistic;
...
/**
 * lemon_perf_log_update_statistic_x:
 * @perf_log: a #LemonPerfLog
 * @name: name of the statistic
 * @value: new value for the statistic
 *
 ...
 */
void
lemon_perf_log_update_statistic_x (LemonPerfLog *perf_log,
                                   const char   *name,
                                   gint64        value)
{
  LemonPerfStatistic *statistic;
...
/**
 * lemon_perf_log_add_statistics_callback:
 * @perf_log: a #LemonPerfLog
 ...
 * and call a function such as lemon_perf_log_update_statistic_i()
 * to update the value that will be recorded.
 */
void
lemon_perf_log_add_statistics_callback (LemonPerfLog               *perf_log,
                                        LemonPerfStatisticsCallback callback,
                                        gpointer                    user_data,
                                        GDestroyNotify              notify)
{
  LemonPerfStatisticsClosure *closure = g_slice_new (LemonPerfStatisticsClosure);
...
/**
 * lemon_perf_log_collect_statistics:
 * @perf_log: a #LemonPerfLog
 *
 * Calls all the update functions added with
 * lemon_perf_log_add_statistics_callback() and then records events
 * for all statistics, followed by a perf.statisticsCollected event.
 */
void
lemon_perf_log_collect_statistics (LemonPerfLog *perf_log)
{
  ...
  for (i = 0; i < perf_log->statistics_closures->len; i++)
    {
      LemonPerfStatisticsClosure *closure;
  ...
  for (i = 0; i < perf_log->statistics->len; i++)
    {
      LemonPerfStatistic *statistic = g_ptr_array_index (perf_log->statistics, i);
      ...
      switch (statistic->event->signature[0])
        ...
        case 'x':
          ...
          break;
        default:
          g_warning("lemon_perf_log_collect_statistics: default case");
          break;
        }
    }
 ...
 */
void
lemon_perf_log_replay (LemonPerfLog            *perf_log,
                       ...
                       gpointer                 user_data)
{
  ...
  for (iter = perf_log->blocks->head; iter; iter = iter->next)
    {
      LemonPerfBlock *block = iter->data;
      guint32 pos = 0;
 ...
 */
gboolean
lemon_perf_log_dump_events (LemonPerfLog   *perf_log,
                            GOutputStream  *out,
                            GError        **error)
{
  GString *output;
  ...
  for (i = 0; i < perf_log->events->len; i++)
    {
      LemonPerfEvent *event = g_ptr_array_index (perf_log->events, i);
 ...
 */
gboolean
lemon_perf_log_dump_log (LemonPerfLog   *perf_log,
                         GOutputStream  *out,
                         GError        **error)
{
  ...
  lemon_perf_log_replay (perf_log, replay_to_json, &closure);
