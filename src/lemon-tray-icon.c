...
G_DEFINE_TYPE (LemonTrayIcon, lemon_tray_icon, LEMON_TYPE_GTK_EMBED);

static void
lemon_tray_icon_finalize (GObject *object)
{
  LemonTrayIcon *icon = LEMON_TRAY_ICON (object);

  g_free (icon->priv->title);
  g_free (icon->priv->wm_class);

  G_OBJECT_CLASS (lemon_tray_icon_parent_class)->finalize (object);
}

static void
lemon_tray_icon_constructed (GObject *object)
{
   ...
   * because Lemon may want to see their values from a
  ...
  if (icon_app_window == NULL)
    {
      g_warning ("lemon tray: icon app window is gone");
      return;
    }
...
static void
lemon_tray_icon_get_property (GObject         *object,
                              guint            prop_id,
                              GValue          *value,
                              GParamSpec      *pspec)
{
...
static void
lemon_tray_icon_class_init (LemonTrayIconClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (LemonTrayIconPrivate));

  object_class->get_property = lemon_tray_icon_get_property;
  object_class->constructed  = lemon_tray_icon_constructed;
  object_class->finalize     = lemon_tray_icon_finalize;
...
static void
lemon_tray_icon_init (LemonTrayIcon *icon)
{
  icon->priv = G_TYPE_INSTANCE_GET_PRIVATE (icon, LEMON_TYPE_TRAY_ICON,
                                            LemonTrayIconPrivate);
}
 ...
 */
ClutterActor *
lemon_tray_icon_new (LemonEmbeddedWindow *window)
{
  g_return_val_if_fail (LEMON_IS_EMBEDDED_WINDOW (window), NULL);

  return g_object_new (LEMON_TYPE_TRAY_ICON,
                       "window", window,
                       NULL);
}
...
/**
 * lemon_tray_icon_handle_event:
 * @icon: a #LemonTrayIcon
 ...
 */
gboolean
lemon_tray_icon_handle_event (LemonTrayIcon *icon,
                                 ...
                                 ClutterEvent     *event)
{
  ...
  if (remote_window == NULL)
    {
      g_warning ("lemon tray: plug window is gone");
