...
#define DESKTOP_SCHEMA "org.lemon.desktop.interface"
...
static GFile *
lemon_util_get_gfile_root (GFile *file)
{
  GFile *parent;
  GFile *parent_old;
...
static char *
lemon_util_get_file_display_name_if_mount (GFile *file)
{
  GFile *compare;
...
static char *
lemon_util_get_file_display_for_common_files (GFile *file)
{
  GFile *compare;
...
static char *
lemon_util_get_file_description (GFile *file)
{
  GFileInfo *info;
  char *ret;
...
static char *
lemon_util_get_file_display_name (GFile *file, gboolean use_fallback)
{
  GFileInfo *info;
  char *ret;
...
static GIcon *
lemon_util_get_file_icon_if_mount (GFile *file)
{
  GFile *compare;
  GVolumeMonitor *monitor;
  GList *mounts, *l;
...
static const char *
lemon_util_get_icon_for_uri_known_folders (const char *uri)
{
  const char *icon;
  char *path;
  int len;
...
char *
lemon_util_get_label_for_uri (const char *text_uri)
{
  GFile *file;
  char  *label;
  GFile *root;
  ...
  label = lemon_util_get_file_display_name_if_mount (file);
  if (label)
    {
      g_object_unref (file);
      return label;
  ...
  if (g_str_has_prefix (text_uri, "file:"))
    {
      label = lemon_util_get_file_display_for_common_files (file);
      if (!label)
        label = lemon_util_get_file_description (file);
      if (!label)
        label = lemon_util_get_file_display_name (file, TRUE);
      g_object_unref (file);
  ...
  root = lemon_util_get_gfile_root (file);
  root_display = lemon_util_get_file_description (root);
  if (!root_display)
    root_display = lemon_util_get_file_display_name (root, FALSE);
  if (!root_display)
    /* can happen with URI schemes non supported by gvfs */
  ...
  if (g_file_equal (file, root))
  ...
  else
    {
      ...
      displayname = lemon_util_get_file_display_name (file, TRUE);
 ...
 */
GIcon *
lemon_util_get_icon_for_uri (const char *text_uri)
{
  const char *name;
  GFile *file;
  GFileInfo *info;
  ...
  name = lemon_util_get_icon_for_uri_known_folders (text_uri);
  if (name)
  ...
  retval = lemon_util_get_file_icon_if_mount (file);
  if (retval)
    {
      g_object_unref (file);
      return retval;
...
/**
 * lemon_util_set_hidden_from_pick:
 ...
 */
void
lemon_util_set_hidden_from_pick (ClutterActor *actor,
                                 gboolean      hidden)
{
  ...
  if (hidden)
    {
      ...
      g_object_set_data (G_OBJECT (actor),
                         "lemon-stop-pick", GUINT_TO_POINTER (1));
    }
  else
    {
      if (existing_handler_data == NULL)
      ...
      g_object_set_data (G_OBJECT (actor), "lemon-stop-pick", NULL);
    }
}

/**
 * lemon_util_get_transformed_allocation:
 * @actor: a #ClutterActor
 ...
 */
void
lemon_util_get_transformed_allocation (ClutterActor    *actor,
                                       ClutterActorBox *box)
{
 ...
 */
char *
lemon_util_format_date (const char *format,
                        gint64      time_ms)
{
  GDateTime *datetime;
  char *result;
...
int
lemon_util_get_week_start (void)
{
 ...
 */
gboolean
lemon_write_string_to_stream (GOutputStream *stream,
                              const char    *str,
                              GError       **error)
{
...
/**
 * lemon_get_file_contents_utf8_sync:
 ...
 */
char *
lemon_get_file_contents_utf8_sync (const char *path,
                                   GError    **error)
{
  char *contents;
  gsize len;
                               ...
                               GCancellable *cancellable)
{
    ...
    contents = lemon_get_file_contents_utf8_sync (path, &error);
...
get_file_contents_utf8_task_finished (GObject      *source,
                                      ...
                                      gpointer      user_data)
{
    LemonFileContentsCallbackData *data = (LemonFileContentsCallbackData *) user_data;
    ...
    if (error)
      {
        g_printerr ("lemon_get_file_contents_utf8 failed: %s\n", error->message);
        g_clear_error (&error);
      }
    ...
    g_slice_free (LemonFileContentsCallbackData, data);
}

/**
 * lemon_get_file_contents_utf8:
 * @path: UTF-8 encoded filename path
 ...
 **/
void
lemon_get_file_contents_utf8         (const char                   *path,
                                         LemonFileContentsCallback  callback,
                                         gpointer                      user_data)
{
  gchar *async_path;
  GTask *task;
  LemonFileContentsCallbackData *data;

  if (path == NULL || callback == NULL)
    {
      g_warning ("lemon_get_file_contents_utf8: path and callback cannot be null");
      return;
    }

  data = g_slice_new (LemonFileContentsCallbackData);
...
/**
 * lemon_util_get_content_for_window_actor:
 ...
 */
ClutterContent *
lemon_util_get_content_for_window_actor (MetaWindowActor *window_actor,
...
cairo_surface_t *
lemon_util_composite_capture_images (ClutterCapture  *captures,
                                        int              n_captures,
                                        int              x,
                                        int              y,
                                        int              target_width,
 ...
 */
void
lemon_breakpoint (void)
{
  G_BREAKPOINT ();
}

/**
 * lemon_parse_search_provider:
 ...
 */
gboolean
lemon_parse_search_provider (const char    *data,
                             char         **name,
                             char         **url,
                             GList        **langs,
                             char         **icon_data_uri,
...
/**
 * lemon_shader_effect_set_double_uniform:
 ...
 */
void
lemon_shader_effect_set_double_uniform (ClutterShaderEffect *effect,
                                        const gchar         *name,
                                        gdouble             value)
{
 ...
 */
gboolean
lemon_util_wifexited (int  status,
                         int *exit)
{
  gboolean ret;
