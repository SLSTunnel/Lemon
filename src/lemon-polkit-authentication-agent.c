...
#include "lemon-polkit-authentication-agent.h"
...
print_debug (const gchar *format, ...)
{
  ...
  g_print ("LemonPolkitAuthenticationAgent: %s.%03d: %s\n",
...
struct _LemonPolkitAuthenticationAgent {
...
G_DEFINE_TYPE (LemonPolkitAuthenticationAgent, lemon_polkit_authentication_agent, POLKIT_AGENT_TYPE_LISTENER);
...
void
lemon_polkit_authentication_agent_init (LemonPolkitAuthenticationAgent *agent)
{
}

gchar *
lemon_polkit_authentication_agent_register (LemonPolkitAuthenticationAgent *agent,
                                            GError                        **error_out)
{
  GError *error = NULL;
...
static void
lemon_polkit_authentication_agent_finalize (GObject *object)
{
  LemonPolkitAuthenticationAgent *agent = LEMON_POLKIT_AUTHENTICATION_AGENT (object);
  lemon_polkit_authentication_agent_unregister (agent);
  G_OBJECT_CLASS (lemon_polkit_authentication_agent_parent_class)->finalize (object);
}

static void
lemon_polkit_authentication_agent_class_init (LemonPolkitAuthenticationAgentClass *klass)
{
  GObjectClass *gobject_class;
  PolkitAgentListenerClass *listener_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = lemon_polkit_authentication_agent_finalize;
...
LemonPolkitAuthenticationAgent *
lemon_polkit_authentication_agent_new (void)
{
  return LEMON_POLKIT_AUTHENTICATION_AGENT (g_object_new (LEMON_TYPE_POLKIT_AUTHENTICATION_AGENT, NULL));
}

struct _AuthRequest {
  /* not holding ref */
  LemonPolkitAuthenticationAgent *agent;
  GCancellable *cancellable;
  gulong handler_id;
                      ...
                      gpointer      user_data)
{
  ...
  g_source_set_name_by_id (id, "[lemon] handle_cancelled_in_idle");
}
...
void
lemon_polkit_authentication_agent_unregister (LemonPolkitAuthenticationAgent *agent)
{
...
static void maybe_process_next_request (LemonPolkitAuthenticationAgent *agent);

static void
auth_request_complete (AuthRequest *request,
                       gboolean     dismissed)
{
  LemonPolkitAuthenticationAgent *agent = request->agent;
...
static void
maybe_process_next_request (LemonPolkitAuthenticationAgent *agent)
{
...
initiate_authentication (PolkitAgentListener  *listener,
                         ...
                         gpointer              user_data)
{
  LemonPolkitAuthenticationAgent *agent = LEMON_POLKIT_AUTHENTICATION_AGENT (listener);
  AuthRequest *request;
...
void
lemon_polkit_authentication_agent_complete (LemonPolkitAuthenticationAgent *agent,
                                            gboolean                        dismissed)
{
  g_return_if_fail (LEMON_IS_POLKIT_AUTHENTICATION_AGENT (agent));
