...
/* lemon-secure-text-buffer.c - secure memory clutter text buffer
...
G_DEFINE_TYPE (LemonSecureTextBuffer, lemon_secure_text_buffer, CLUTTER_TYPE_TEXT_BUFFER);

static const gchar *
lemon_secure_text_buffer_real_get_text (ClutterTextBuffer *buffer,
                                           gsize             *n_bytes)
{
  LemonSecureTextBuffer *self = LEMON_SECURE_TEXT_BUFFER (buffer);
  if (n_bytes)
...
static guint
lemon_secure_text_buffer_real_get_length (ClutterTextBuffer *buffer)
{
  LemonSecureTextBuffer *self = LEMON_SECURE_TEXT_BUFFER (buffer);
...
static guint
lemon_secure_text_buffer_real_insert_text (ClutterTextBuffer *buffer,
                                              guint              position,
                                              const gchar       *chars,
                                              guint              n_chars)
{
...
static guint
lemon_secure_text_buffer_real_delete_text (ClutterTextBuffer *buffer,
                                              guint              position,
                                              guint              n_chars)
{
  LemonSecureTextBuffer *self = LEMON_SECURE_TEXT_BUFFER (buffer);
...
static void
lemon_secure_text_buffer_init (LemonSecureTextBuffer *self)
{

}

static void
lemon_secure_text_buffer_finalize (GObject *obj)
{
  LemonSecureTextBuffer *self = LEMON_SECURE_TEXT_BUFFER (obj);
  ...
  G_OBJECT_CLASS (lemon_secure_text_buffer_parent_class)->finalize (obj);
}

static void
lemon_secure_text_buffer_class_init (LemonSecureTextBufferClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  ClutterTextBufferClass *buffer_class = CLUTTER_TEXT_BUFFER_CLASS (klass);

  gobject_class->finalize = lemon_secure_text_buffer_finalize;

  buffer_class->get_text = lemon_secure_text_buffer_real_get_text;
  buffer_class->get_length = lemon_secure_text_buffer_real_get_length;
  buffer_class->insert_text = lemon_secure_text_buffer_real_insert_text;
  buffer_class->delete_text = lemon_secure_text_buffer_real_delete_text;
}

ClutterTextBuffer *
lemon_secure_text_buffer_new (void)
{
  return g_object_new (LEMON_TYPE_SECURE_TEXT_BUFFER, NULL);
}
