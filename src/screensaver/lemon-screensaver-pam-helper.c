...
shutdown (void)
{
    DEBUG ("lemon-screensaver-pam-helper (pid %i): shutting down.\n", getpid ());
                      ...
                      gpointer           data)
{
    ...
    DEBUG ("lemon-screensaver-pam-helper: Got message style %d: '%s'\n", style, msg);
    ret = TRUE;
    *response = NULL;
    ...
    switch (style)
    {
        case CS_AUTH_MESSAGE_PROMPT_ECHO_ON:
            DEBUG ("lemon-screensaver-pam-helper: CS_AUTH_MESSAGE_PROMPT_ECHO_ON\n");
            break;
        case CS_AUTH_MESSAGE_PROMPT_ECHO_OFF:
            if (msg != NULL)
            {
                ...
                DEBUG ("lemon-screensaver-pam-helper: auth_message_handler processing response string\n");
    ...
    {
        if (*response == NULL) {
            DEBUG ("lemon-screensaver-pam-helper: Got no response to prompt\n");
            ret = FALSE;
        } else {
            send_busy (TRUE);
        }
...
do_auth_check (void)
{
    ...
    DEBUG ("lemon-screensaver-pam-helper: Verify user returned: %s\n", res ? "TRUE" : "FALSE");

    if (!res)
    {
        if (error != NULL && !g_cancellable_is_cancelled (stdin_cancellable))
        {
            DEBUG ("lemon-screensaver-pam-helper: Verify user returned error: %s\n", error->message);
...
auth_check_idle (gpointer user_data)
{
    ...
    else
    {
        ...
        {
            send_failure ();
            DEBUG ("lemon-screensaver-pam-helper: Authentication failed, retrying (%u)\n", loop_counter);
        }
        else
        {
            DEBUG ("lemon-screensaver-pam-helper: Authentication failed, quitting (max failures)\n");
            again = FALSE;
            send_cancelled ();
        }
    }
                           ...
                           gpointer      user_data)
{
    ...
    if (error != NULL)
    {
        ...
        {
            g_critical ("lemon-screensaver-pam-helper: stdin monitor: Could not read input from lemon-screensaver: %s", error->message);
        }
        g_error_free (error);
    }

    DEBUG ("lemon-screensaver-pam-helper: stdin_monitor_task_finished (Cancelled: %d)\n",
                           ...
                           gboolean verbose)
{
    ...
    if (nolock_reason)
    {
        DEBUG ("lemon-screensaver-pam-helper: Locking disabled: %s\n", nolock_reason);
    }

    if (uid_message && verbose)
    {
        g_print ("lemon-screensaver-pam-helper: Modified UID: %s", uid_message);
    }
...
{
    DEBUG ("lemon-screensaver-pam-helper (pid %i): SIGTERM, shutting down\n", getpid ());
      ...
      char **argv)
{
    ...
    context = g_option_context_new (N_("\n\nPAM interface for lemon-screensaver."));
    ...
    if (! lock_initialization (&argc, argv, &nolock_reason, debug_mode))
    {
        if (nolock_reason != NULL)
        {
            DEBUG ("lemon-screensaver-pam-helper: Screen locking disabled: %s\n", nolock_reason);
            g_free (nolock_reason);
        }
        response_lock_init_failed ();
    ...
    DEBUG ("lemon-screensaver-pam-helper (pid %i): start\n", getpid ());
    ...
    DEBUG ("lemon-screensaver-pam-helper: exit\n");
    return 0;
}
