...
struct _LemonWindowTracker
{
  ...
  /* <MetaWindow * window, LemonApp *app> */
...
G_DEFINE_TYPE (LemonWindowTracker, lemon_window_tracker, G_TYPE_OBJECT);
...
static void lemon_window_tracker_finalize (GObject *object);
static void set_focus_app (LemonWindowTracker  *tracker,
                           LemonApp            *new_focus_app);
static void on_focus_window_changed (MetaDisplay *display, GParamSpec *spec, LemonWindowTracker *tracker);

static void track_window (LemonWindowTracker *tracker, MetaWindow *window);
static void disassociate_window (LemonWindowTracker *tracker, MetaWindow *window);
LemonApp *meta_startup_sequence_get_app (MetaStartupSequence *sequence);

static void
lemon_window_tracker_get_property (GObject    *gobject,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  LemonWindowTracker *tracker = LEMON_WINDOW_TRACKER (gobject);
...
static void
lemon_window_tracker_class_init (LemonWindowTrackerClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = lemon_window_tracker_get_property;
  gobject_class->finalize = lemon_window_tracker_finalize;
...
/**
 * lemon_window_tracker_is_window_interesting:
 * @tracker: the LemonWindowTracker
 * @window: a #MetaWindow
 *
 * The LemonWindowTracker associates certain kinds of windows with
 ...
 */
gboolean
lemon_window_tracker_is_window_interesting (LemonWindowTracker *tracker, MetaWindow *window)
{
    MetaWindowType type;
  ...
  switch (type)
    {
      ...
      case META_WINDOW_UTILITY:
        break;
      default:
        g_warning ("lemon_window_tracker_is_window_interesting: default reached");
      break;
    }
 ...
 * return %NULL.
 *
 * Return value: (transfer full): A newly-referenced #LemonApp, or %NULL
 */
static LemonApp *
get_app_from_window_wmclass (MetaWindow  *window)
{
  LemonApp *app;
  LemonAppSystem *appsys;
  const char *wm_class;
  const char *wm_instance;

  appsys = lemon_app_system_get_default ();
  ...
  */
  ...
  wm_instance = meta_window_get_wm_class_instance (window);
  app = lemon_app_system_lookup_startup_wmclass (appsys, wm_instance);
  if (app != NULL)
    return g_object_ref (app);

  /* then try a match from WM_CLASS to StartupWMClass */
  wm_class = meta_window_get_wm_class (window);
  app = lemon_app_system_lookup_startup_wmclass (appsys, wm_class);
  if (app != NULL)
    return g_object_ref (app);

  /* then try a match from WM_CLASS (instance part) to .desktop */
  app = lemon_app_system_lookup_desktop_wmclass (appsys, wm_instance);
  if (app != NULL)
    return g_object_ref (app);
  ...
  app = lemon_app_system_lookup_desktop_wmclass (appsys, wm_class);
  if (app != NULL)
    return g_object_ref (app);
...
get_app_from_gapplication_id (MetaWindow  *window)
{
  ...
  app = lemon_app_system_lookup_app (appsys, desktop_file);
  if (app)
    g_object_ref (app);
 ...
 */
static LemonApp*
get_app_from_window_group (LemonWindowTracker  *tracker,
                           MetaWindow          *window)
{
  LemonApp *result;
 ...
 */
static LemonApp *
get_app_from_window_pid (LemonWindowTracker  *tracker,
                         MetaWindow          *window)
{
  LemonApp *result;
  int pid;
  ...
  result = lemon_window_tracker_get_app_from_pid (tracker, pid);
  if (result != NULL)
    g_object_ref (result);
  ...
  if (g_file_query_exists (file, NULL)) {
    ...
    if (result == NULL) {
      ...
      if (g_key_file_load_from_file (keyfile, info_filename, G_KEY_FILE_NONE, NULL)) {
        ...
        app = lemon_app_system_lookup_flatpak_app_id (appsys, app_id);
 ...
 *
 * Returns: (transfer full): a #LemonApp, or NULL if none is found
 */
static LemonApp *
get_app_for_window (LemonWindowTracker    *tracker,
                    MetaWindow            *window)
{
  LemonApp *result = NULL;
  ...
  if (meta_window_is_remote (window))
    return _lemon_app_new_for_window (window);
  ...
  if (startup_id)
    {
      ...
      sequences = lemon_window_tracker_get_startup_sequences (tracker);
  ...
  if (result == NULL)
    result = _lemon_app_new_for_window (window);

  return result;
}

static void
update_focus_app (LemonWindowTracker *self)
{
  MetaWindow *new_focus_win;
  LemonApp *new_focus_app;

  new_focus_win = meta_display_get_focus_window (self->global->meta_display);
  new_focus_app = new_focus_win ? lemon_window_tracker_get_window_app (self, new_focus_win) : NULL;
...
static void
tracked_window_changed (LemonWindowTracker *self,
                        MetaWindow         *window)
{
...
on_wm_class_changed (MetaWindow  *window,
                     ...
                     gpointer     user_data)
{
  LemonWindowTracker *self = LEMON_WINDOW_TRACKER (user_data);
                 ...
                 gpointer    user_data)
{
  ...
  if (old_app == NULL || !lemon_app_is_window_backed (old_app))
    return;

  new_app = get_app_for_window (self, window);

  if (new_app != NULL && new_app != old_app && !lemon_app_is_window_backed (new_app))
    {
      _lemon_app_remove_window (old_app, window);
      g_hash_table_replace (self->window_to_app, window, new_app);
      _lemon_app_add_window (new_app, window);
...
static void
track_window (LemonWindowTracker *self,
              MetaWindow      *window)
{
  LemonApp *app;

  if (!lemon_window_tracker_is_window_interesting (self, window))
    return;
  ...
  if (meta_window_get_client_type (window) == META_WINDOW_CLIENT_TYPE_WAYLAND &&
      lemon_app_is_window_backed (app))
    {
...
static void
lemon_window_tracker_on_window_added (MetaWorkspace   *workspace,
                                   MetaWindow      *window,
                                   gpointer         user_data)
{
  LemonWindowTracker *self = LEMON_WINDOW_TRACKER (user_data);
...
static void
disassociate_window (LemonWindowTracker   *self,
                     MetaWindow        *window)
{
  LemonApp *app;
...
static void
lemon_window_tracker_on_window_removed (MetaWorkspace   *workspace,
                                     MetaWindow      *window,
                                     gpointer         user_data)
{
  disassociate_window (LEMON_WINDOW_TRACKER (user_data), window);
}

void
load_initial_windows (LemonWindowTracker *tracker)
{
...
static void
lemon_window_tracker_on_n_workspaces_changed (MetaWorkspaceManager *workspace_manager,
                                              GParamSpec           *pspec,
                                              gpointer              user_data)
{
  LemonWindowTracker *self = LEMON_WINDOW_TRACKER (user_data);
  ...
  for (l = workspaces; l; l = l->next)
    {
      ...
      g_signal_handlers_disconnect_by_func (workspace,
                                            lemon_window_tracker_on_window_added,
                                            self);
      g_signal_handlers_disconnect_by_func (workspace,
                                            lemon_window_tracker_on_window_removed,
                                            self);

      g_signal_connect (workspace, "window-added",
                        G_CALLBACK (lemon_window_tracker_on_window_added), self);
      g_signal_connect (workspace, "window-removed",
                        G_CALLBACK (lemon_window_tracker_on_window_removed), self);
    }
}

static void
init_window_tracking (LemonWindowTracker *self)
{
...
static void
lemon_window_tracker_finalize (GObject *object)
{
  LemonWindowTracker *self = LEMON_WINDOW_TRACKER (object);

  g_hash_table_destroy (self->window_to_app);
  g_hash_table_destroy (self->launched_pid_to_app);

  G_OBJECT_CLASS (lemon_window_tracker_parent_class)->finalize(object);
}

/**
 * lemon_window_tracker_get_window_app:
 ...
 */
LemonApp *
lemon_window_tracker_get_window_app (LemonWindowTracker *tracker,
                                     MetaWindow         *metawin)
{
...
/**
 * lemon_window_tracker_get_app_from_pid:
 * @self; A #LemonAppSystem
 * @pid: A Unix process identifier
 *
 * Look up the application corresponding to a process.
 *
 * Returns: (transfer none): A #LemonApp, or %NULL if none
 */
LemonApp *
lemon_window_tracker_get_app_from_pid (LemonWindowTracker *self,
                                       int                 pid)
{
  GSList *running = lemon_app_system_get_running (lemon_app_system_get_default());
  GSList *iter;
  LemonApp *result = NULL;

  for (iter = running; iter; iter = iter->next)
    {
      LemonApp *app = iter->data;
      GSList *pids = lemon_app_get_pids (app);
      GSList *pids_iter;
...
void
_lemon_window_tracker_add_child_process_app (LemonWindowTracker *tracker,
                                             GPid                pid,
                                             LemonApp           *app)
{
...
static void
set_focus_app (LemonWindowTracker  *tracker,
               LemonApp            *new_focus_app)
{
...
/**
 * lemon_window_tracker_get_startup_sequences:
 * @tracker:
 *
 * Returns: (transfer none) (element-type MetaStartupSequence): Currently active startup sequences
 */
GSList *
lemon_window_tracker_get_startup_sequences (LemonWindowTracker *self)
{
  MetaStartupNotification *sn = meta_display_get_startup_notification (self->global->meta_display);
...
meta_startup_sequence_get_app (MetaStartupSequence *sequence)
{
  ...
  appsys = lemon_app_system_get_default ();
  app = lemon_app_system_lookup_app (appsys, basename);
  g_free (basename);
  return app;
}
...
/**
 * lemon_window_tracker_get_default:
 *
 * Return Value: (transfer none): The global #LemonWindowTracker instance
 */
LemonWindowTracker *
lemon_window_tracker_get_default (void)
{
  static LemonWindowTracker *instance;

  if (instance == NULL)
    instance = g_object_new (LEMON_TYPE_WINDOW_TRACKER, NULL);
