...
#include "lemon-screenshot.h"

struct _LemonScreenshotClass
{
  GObjectClass parent_class;
};

struct _LemonScreenshot
{
...
typedef struct _screenshot_data {
  ...
  LemonScreenshotPickColorCallback pick_callback;
  ClutterColor pick_color;
} _screenshot_data;

G_DEFINE_TYPE(LemonScreenshot, lemon_screenshot, G_TYPE_OBJECT);

static void
lemon_screenshot_class_init (LemonScreenshotClass *screenshot_class)
{
  (void) screenshot_class;
}

static void
lemon_screenshot_init (LemonScreenshot *screenshot)
{
  screenshot->global = lemon_global_get ();
}
                   ...
                   cairo_rectangle_int_t  area)
{
  ...
  display = lemon_global_get_display (lemon_global_get ());
...
{
  MetaDisplay *display = lemon_global_get_display (screenshot_data->screenshot->global);
                        ...
                        _screenshot_data *screenshot_data)
{
  ...
  result = g_simple_async_result_new (NULL, on_screenshot_written, (gpointer)screenshot_data, lemon_screenshot_screenshot_window);
  g_simple_async_result_run_in_thread (result, write_screenshot_thread, G_PRIORITY_DEFAULT, NULL);
  g_object_unref (result);
}
...
/**
 * lemon_screenshot_screenshot:
 * @screenshot: the #LemonScreenshot
 ...
 */
void
lemon_screenshot_screenshot (LemonScreenshot *screenshot,
                             gboolean include_cursor,
                             const char *filename,
                             LemonScreenshotCallback callback)
{
  ...
  display = lemon_global_get_display (screenshot->global);
  stage = CLUTTER_ACTOR (lemon_global_get_stage (screenshot->global));
...
/**
 * lemon_screenshot_screenshot_area:
 * @screenshot: the #LemonScreenshot
 ...
 */
void
lemon_screenshot_screenshot_area (LemonScreenshot *screenshot,
                                  gboolean include_cursor,
                                  int x,
                                  int y,
                                  int width,
...
/**
 * lemon_screenshot_screenshot_window:
 * @screenshot: the #LemonScreenshot
 ...
 */
void
lemon_screenshot_screenshot_window (LemonScreenshot *screenshot,
                                    gboolean include_frame,
                                    gboolean include_cursor,
                                    const char *filename,
                                    LemonScreenshotCallback callback)
{
  MetaDisplay *display = lemon_global_get_display (screenshot->global);
  MetaWindow *window = meta_display_get_focus_window (display);
  ClutterActor *stage;

  if (window == NULL || g_strcmp0 (meta_window_get_title (window), "Desktop") == 0)
  {
    lemon_screenshot_screenshot (screenshot, include_cursor, filename, callback);
    return;
  }
 ...
 */
void
lemon_screenshot_pick_color (LemonScreenshot *screenshot,
                                int x,
                                int y,
...
LemonScreenshot *
lemon_screenshot_new (void)
{
  return g_object_new (LEMON_TYPE_SCREENSHOT, NULL);
}
