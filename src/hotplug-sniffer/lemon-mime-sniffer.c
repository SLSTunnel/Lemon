...
G_DEFINE_TYPE (LemonMimeSniffer, lemon_mime_sniffer, G_TYPE_OBJECT);
...
struct _LemonMimeSnifferPrivate {
  GFile *file;
...
{
  LemonMimeSniffer *self = state->self;
  GArray *results;
...
static void
deep_count_start (LemonMimeSniffer *self)
{
  DeepCountState *state;
                           ...
                           gpointer user_data)
{
  GFileInfo *info;
  GError *error = NULL;
  LemonMimeSniffer *self = user_data;
...
static void
start_loading_file (LemonMimeSniffer *self)
{
...
static void
lemon_mime_sniffer_set_file (LemonMimeSniffer *self,
                            GFile *file)
{
...
static void
lemon_mime_sniffer_dispose (GObject *object)
{
  LemonMimeSniffer *self = LEMON_MIME_SNIFFER (object);
  ...
  G_OBJECT_CLASS (lemon_mime_sniffer_parent_class)->dispose (object);
}

static void
lemon_mime_sniffer_finalize (GObject *object)
{
  LemonMimeSniffer *self = LEMON_MIME_SNIFFER (object);

  g_strfreev (self->priv->sniffed_mime);

  G_OBJECT_CLASS (lemon_mime_sniffer_parent_class)->finalize (object);
}

static void
lemon_mime_sniffer_get_property (GObject *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
...
static void
lemon_mime_sniffer_set_property (GObject *object,
                                guint       prop_id,
                                const GValue *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
  case PROP_FILE:
    lemon_mime_sniffer_set_file (self, g_value_get_object (value));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    break;
...
static void
lemon_mime_sniffer_class_init (LemonMimeSnifferClass *klass)
{
  GObjectClass *oclass;

  oclass = G_OBJECT_CLASS (klass);
  oclass->dispose = lemon_mime_sniffer_dispose;
  oclass->finalize = lemon_mime_sniffer_finalize;
  oclass->get_property = lemon_mime_sniffer_get_property;
  oclass->set_property = lemon_mime_sniffer_set_property;
  ...
  g_type_class_add_private (klass, sizeof (LemonMimeSnifferPrivate));
  g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

static void
lemon_mime_sniffer_init (LemonMimeSniffer *self)
{
  self->priv =
    G_TYPE_INSTANCE_GET_PRIVATE (self,
                                 LEMON_TYPE_MIME_SNIFFER,
                                 LemonMimeSnifferPrivate);
  init_mimetypes ();
}

LemonMimeSniffer *
lemon_mime_sniffer_new (GFile *file)
{
  return g_object_new (LEMON_TYPE_MIME_SNIFFER,
                       "file", file,
                       NULL);
}

void
lemon_mime_sniffer_sniff_async (LemonMimeSniffer *self,
                                ...
                                gpointer user_data)
{
  ...
  self->priv->async_result =
    g_simple_async_result_new (G_OBJECT (self),
                               callback, user_data,
                               lemon_mime_sniffer_sniff_finish);
...
gchar **
lemon_mime_sniffer_sniff_finish (LemonMimeSniffer *self,
                                 GAsyncResult *res,
                                 GError **error)
{
