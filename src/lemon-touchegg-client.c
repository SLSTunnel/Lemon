...
#include "lemon-touchegg-client.h"
...
} LemonToucheggClientPrivate;

struct _LemonToucheggClient
{
    GObject parent_instance;
};

G_DEFINE_TYPE_WITH_PRIVATE (LemonToucheggClient, lemon_touchegg_client, G_TYPE_OBJECT)
...
static void
emit_our_signal (LemonToucheggClient *client,
                 ...
                 GVariant               *params)
{
    ...
    g_debug ("LemonToucheggClient signal: %s: type %u, direction %u, progress %0.1f, fingers %d, device %u, elapsed_time %lu",
...
handle_signal (GDBusConnection *connection,
               ...
               gpointer         user_data)
{
    g_return_if_fail (LEMON_IS_TOUCHEGG_CLIENT (user_data));
    LemonToucheggClient *client = LEMON_TOUCHEGG_CLIENT (user_data);
    LemonToucheggClientPrivate *priv = lemon_touchegg_client_get_instance_private (client);
...
static void init_connection (LemonToucheggClient *client);

static gboolean
retry_connection (gpointer data)
{
    LemonToucheggClient *client = LEMON_TOUCHEGG_CLIENT (data);
    LemonToucheggClientPrivate *priv = lemon_touchegg_client_get_instance_private (client);

    g_debug ("LemonToucheggClient: retrying connection");
...
static void
connect_listener (LemonToucheggClient *client)
{
    LemonToucheggClientPrivate *priv = lemon_touchegg_client_get_instance_private (client);
    g_debug ("LemonToucheggClient: connect_listener");
...
static void
init_connection (LemonToucheggClient *client)
{
    LemonToucheggClientPrivate *priv = lemon_touchegg_client_get_instance_private (client);
    g_debug ("LemonToucheggClient: init_client");
...
static void
lemon_touchegg_client_init (LemonToucheggClient *client)
{
    init_connection (client);
}

static void
lemon_touchegg_client_dispose (GObject *object)
{
    LemonToucheggClient *client = LEMON_TOUCHEGG_CLIENT (object);
    LemonToucheggClientPrivate *priv = lemon_touchegg_client_get_instance_private (client);
    g_debug ("LemonToucheggClient dispose (%p)", object);
    ...
    G_OBJECT_CLASS (lemon_touchegg_client_parent_class)->dispose (object);
}

static void
lemon_touchegg_client_finalize (GObject *object)
{
    g_debug ("LemonToucheggClient finalize (%p)", object);

    G_OBJECT_CLASS (lemon_touchegg_client_parent_class)->finalize (object);
}

static void
lemon_touchegg_client_class_init (LemonToucheggClientClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->dispose = lemon_touchegg_client_dispose;
    gobject_class->finalize = lemon_touchegg_client_finalize;

    signals[GESTURE_BEGIN] =
          g_signal_new ("gesture-begin",
                        LEMON_TYPE_TOUCHEGG_CLIENT,
...
LemonToucheggClient *
lemon_touchegg_client_new (void)
{
    LemonToucheggClient *client = g_object_new (LEMON_TYPE_TOUCHEGG_CLIENT, NULL);
