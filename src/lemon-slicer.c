...
#include "lemon-slicer.h"

G_DEFINE_TYPE (LemonSlicer,
               lemon_slicer,
               ST_TYPE_BIN);

static void
lemon_slicer_get_preferred_width (ClutterActor *self,
                                  gfloat        for_height,
                                  gfloat       *min_width_p,
...
static void
lemon_slicer_get_preferred_height (ClutterActor *self,
                                   gfloat        for_width,
                                   gfloat       *min_height_p,
...
static void
lemon_slicer_allocate (ClutterActor           *self,
...
static void
lemon_slicer_paint_pick_child (LemonSlicer  *self,
                                  CoglFramebuffer *fb,
                                  gpointer         ctx,
                                  gboolean         pick)
{
...
static void
lemon_slicer_paint (ClutterActor *self, ClutterPaintContext *paint_context)
{
  ...
  lemon_slicer_paint_pick_child (LEMON_SLICER (self), fb, paint_context, FALSE);
}

static void
lemon_slicer_pick (ClutterActor       *self,
                      ClutterPickContext *pick_context)
{
  CoglFramebuffer *fb = clutter_pick_context_get_framebuffer (pick_context);
  lemon_slicer_paint_pick_child (LEMON_SLICER (self), fb, pick_context, TRUE);
}

static void
lemon_slicer_class_init (LemonSlicerClass *klass)
{
  ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS (klass);

  actor_class->get_preferred_width = lemon_slicer_get_preferred_width;
  actor_class->get_preferred_height = lemon_slicer_get_preferred_height;
  actor_class->allocate = lemon_slicer_allocate;
  actor_class->paint = lemon_slicer_paint;
  actor_class->pick = lemon_slicer_pick;
}

static void
lemon_slicer_init (LemonSlicer *actor)
{
}
