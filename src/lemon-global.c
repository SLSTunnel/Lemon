...
#include <liblemon-desktop/gnome-systemd.h>
...
static LemonGlobal *the_object = NULL;
...
G_DEFINE_TYPE(LemonGlobal, lemon_global, G_TYPE_OBJECT);

static guint lemon_global_signals [LAST_SIGNAL] = { 0 };

static void setup_log_handler (LemonGlobal *global);

static void
lemon_global_set_property(GObject         *object,
                          guint            prop_id,
                          const GValue    *value,
                          GParamSpec      *pspec)
{
  LemonGlobal *global = LEMON_GLOBAL (object);

  switch (prop_id)
    {
    case PROP_STAGE_INPUT_MODE:
      lemon_global_set_stage_input_mode (global, g_value_get_enum (value));
      break;
...
static void
lemon_global_get_property(GObject         *object,
                          guint            prop_id,
                          GValue          *value,
                          GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_OVERLAY_GROUP:
      ...
      break;
    case PROP_SCREEN:
      g_value_set_object (value, global->lemon_screen);
      break;
    case PROP_DISPLAY:
...
static void
setup_notifications_service (LemonGlobal *global)
{
  gboolean disabled;

  /* notify-osd allows itself to be replaced as the notification handler. dunst does not,
     nor does lemon. If we attempt to own and fail, lemon is still 'on deck' to take
...
static void
lemon_global_init (LemonGlobal *global)
{
  const char *datadir = g_getenv ("LEMON_DATADIR");
  const char *lemon_js = g_getenv("LEMON_JS");
  char *imagedir, **search_path;

  if (!datadir)
    datadir = LEMON_DATADIR;
  ...
  global->userdatadir = g_build_filename (g_get_user_data_dir (), "lemon", NULL);
  g_mkdir_with_parents (global->userdatadir, 0700);

  global->settings = g_settings_new ("org.lemon");
  ...
  global->input_mode = LEMON_STAGE_INPUT_MODE_NORMAL;

  if (!lemon_js)
    lemon_js = JSDIR;
  search_path = g_strsplit (lemon_js, ":", -1);
...
static void
lemon_global_finalize (GObject *object)
{
  LemonGlobal *global = LEMON_GLOBAL (object);
  ...
  G_OBJECT_CLASS(lemon_global_parent_class)->finalize (object);
}

static void
lemon_global_class_init (LemonGlobalClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = lemon_global_get_property;
  gobject_class->set_property = lemon_global_set_property;
  gobject_class->finalize = lemon_global_finalize;

  lemon_global_signals[NOTIFY_ERROR] =
  ...
  lemon_global_signals[SCALE_CHANGED] =
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_object ("display",
                                                        "Display",
                                                        "Metacity display object for Lemon",
                                                        META_TYPE_DISPLAY,
                                                        G_PARAM_READABLE));
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_object ("settings",
                                                        "Settings",
                                                        "GSettings instance for Lemon configuration",
                                                        G_TYPE_SETTINGS,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_DATADIR,
                                   g_param_spec_string ("datadir",
                                                        "Data directory",
                                                        "Directory containing Lemon data files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_IMAGEDIR,
                                   g_param_spec_string ("imagedir",
                                                        "Image directory",
                                                        "Directory containing Lemon image files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_USERDATADIR,
                                   g_param_spec_string ("userdatadir",
                                                        "User data directory",
                                                        "Directory containing Lemon user data",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_FOCUS_MANAGER,
...
/**
 * _lemon_global_init: (skip)
 * @first_property_name: the name of the first property
 * @...: the value of the first property, followed optionally by more
 *  name/value pairs, followed by %NULL
 *
 * Initializes Lemon global singleton with the construction-time
 * properties.
 *
 * There are currently no such properties, so @first_property_name should
 * always be %NULL.
 *
 * This call must be called before lemon_global_get() and shouldn't be called
 * more than once.
 */
void
_lemon_global_init (const char *first_property_name,
                    ...)
{
  ...
  the_object = LEMON_GLOBAL (g_object_new_valist (LEMON_TYPE_GLOBAL,
                                                  first_property_name,
                                                  argument_list));
...
focus_window_changed (MetaDisplay *display,
                      ...
                      gpointer     user_data)
{
  LemonGlobal *global = user_data;

  if (global->input_mode == LEMON_STAGE_INPUT_MODE_FOCUSED &&
      meta_display_get_focus_window (display) != NULL)
    lemon_global_set_stage_input_mode (global, LEMON_STAGE_INPUT_MODE_NORMAL);
}

static void
lemon_global_focus_stage (LemonGlobal *global)
{
  meta_focus_stage_window (global->meta_display, lemon_global_get_current_time (global));
}

/**
 * lemon_global_set_stage_input_mode:
 * @global: the #LemonGlobal
 * @mode: the stage input mode
 *
 * Sets the input mode of the stage; when @mode is
 * %LEMON_STAGE_INPUT_MODE_NONREACTIVE, then the stage does not absorb
 * any clicks, but just passes them through to underlying windows.
 * When it is %LEMON_STAGE_INPUT_MODE_NORMAL, then the stage accepts
 * clicks in the region defined by
 * lemon_global_set_stage_input_region() but passes through clicks
 * outside that region. When it is %LEMON_STAGE_INPUT_MODE_FULLSCREEN,
 * the stage absorbs all input.
 *
 * When the input mode is %LEMON_STAGE_INPUT_MODE_FOCUSED, the pointer
 * is handled as with %LEMON_STAGE_INPUT_MODE_NORMAL, but additionally
 ...
 * will revert to %LEMON_STAGE_INPUT_MODE_NORMAL.
 *
 * Note that whenever a muffin-internal Gtk widget has a pointer grab,
 * Lemon behaves as though it was in
 * %LEMON_STAGE_INPUT_MODE_NONREACTIVE, to ensure that the widget gets
 * any clicks it is expecting.
 */
void
lemon_global_set_stage_input_mode (LemonGlobal         *global,
                                      LemonStageInputMode  mode)
{
  g_return_if_fail (LEMON_IS_GLOBAL (global));
  ...
  if (mode == LEMON_STAGE_INPUT_MODE_NONREACTIVE)
    meta_x11_display_clear_stage_input_region (x11_display);
  else if (mode == LEMON_STAGE_INPUT_MODE_FULLSCREEN || !global->input_region)
  ...
  if (mode == LEMON_STAGE_INPUT_MODE_FOCUSED)
    lemon_global_focus_stage (global);
...
/**
 * lemon_global_set_cursor:
 * @global: A #LemonGlobal
 * @type: the type of the cursor
 *
 * Set the cursor on the stage window.
 */
void
lemon_global_set_cursor (LemonGlobal *global,
                            LemonCursor type)
{
  MetaCursor ret_curs;
  ...
  switch (type)
    {
    case LEMON_CURSOR_NOT_ALLOWED:
      ret_curs = META_CURSOR_NOT_ALLOWED;
      break;
    case LEMON_CURSOR_MOVE:
      ret_curs = META_CURSOR_MOVE;
      break;
    case LEMON_CURSOR_COPY:
      ret_curs = META_CURSOR_COPY;
      break;
    case LEMON_CURSOR_NO_DROP:
      ret_curs = META_CURSOR_NO_DROP;
      break;
    case LEMON_CURSOR_POINTER:
      ret_curs = META_CURSOR_POINTER;
      break;
    case LEMON_CURSOR_RESIZE_BOTTOM:
      ret_curs = META_CURSOR_S_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_TOP:
      ret_curs = META_CURSOR_N_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_LEFT:
      ret_curs = META_CURSOR_W_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_RIGHT:
      ret_curs = META_CURSOR_E_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_BOTTOM_RIGHT:
      ret_curs = META_CURSOR_SE_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_BOTTOM_LEFT:
      ret_curs = META_CURSOR_SW_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_TOP_RIGHT:
      ret_curs = META_CURSOR_NE_RESIZE;
      break;
    case LEMON_CURSOR_RESIZE_TOP_LEFT:
      ret_curs = META_CURSOR_NW_RESIZE;
      break;
    case LEMON_CURSOR_CROSSHAIR:
      ret_curs = META_CURSOR_CROSSHAIR;
      break;
    case LEMON_CURSOR_TEXT:
...
/**
 * lemon_global_unset_cursor:
 * @global: A #LemonGlobal
 *
 * Unset the cursor on the stage window.
 */
void
lemon_global_unset_cursor (LemonGlobal  *global)
{
  meta_display_set_cursor (global->meta_display, META_CURSOR_DEFAULT);
}

/**
 * lemon_global_set_stage_input_region:
 * @global: the #LemonGlobal
 * @rectangles: (element-type Meta.Rectangle): a list of #MetaRectangle
 * describing the input region.
 *
 * Sets the area of the stage that is responsive to mouse clicks when
 * the stage mode is %LEMON_STAGE_INPUT_MODE_NORMAL (but does not change the
 * current stage mode).
 */
void
lemon_global_set_stage_input_region (LemonGlobal *global,
                                     GSList      *rectangles)
{
  MetaRectangle *rect;
  XRectangle *rects;
   ...
   */
  lemon_global_set_stage_input_mode (global, global->input_mode);
}

/**
 * lemon_global_get_stage:
 *
 * Return value: (transfer none): The default #ClutterStage
 */
ClutterStage *
lemon_global_get_stage (LemonGlobal  *global)
{
  return global->stage;
}

/**
 * lemon_global_get_screen:
 *
 * Return value: (transfer none): The (Meta) LemonScreen
 */
LemonScreen *
lemon_global_get_screen (LemonGlobal  *global)
{
  return global->lemon_screen;
}

/**
 * lemon_global_get_display:
 *
 * Return value: (transfer none): The default #MetaDisplay
 */
MetaDisplay *
lemon_global_get_display (LemonGlobal  *global)
{
  return global->meta_display;
}

/**
 * lemon_global_get_window_actors:
 *
 ...
 */
GList *
lemon_global_get_window_actors (LemonGlobal *global)
{
  g_return_val_if_fail (LEMON_IS_GLOBAL (global), NULL);
...
global_stage_notify_width (GObject    *gobject,
                           ...
                           gpointer    data)
{
  LemonGlobal *global = LEMON_GLOBAL (data);
...
{
  lemon_perf_log_event (lemon_perf_log_get_default (),
...
static void
lemon_fonts_init (ClutterStage *stage)
{
...
static void
update_scaling_factor (LemonGlobal  *global,
...
void
_lemon_global_set_plugin (LemonGlobal *global,
                          MetaPlugin  *plugin)
{
  g_return_if_fail (LEMON_IS_GLOBAL (global));
  ...
  global->lemon_screen = lemon_screen_new (global->meta_display);
  ...
  if (g_getenv ("LEMON_PERF_OUTPUT") != NULL)
    {
  ...
  // gdk_event_handler_set (gnome_lemon_gdk_event_handler, global->stage, NULL);
...
/**
 * lemon_global_dump_gjs_stack:
 * @global: A #LemonGlobal
 *
 * Prints out the gjs stack
 */
void
lemon_global_dump_gjs_stack (LemonGlobal *global)
{
  gjs_dumpstack ();
}

GjsContext *
_lemon_global_get_gjs_context (LemonGlobal *global)
{
...
static void
setup_log_handler (LemonGlobal *global)
{
...
static guint32
get_current_time_maybe_roundtrip (LemonGlobal *global)
{
  guint32 time;

  time = lemon_global_get_current_time (global);
  if (time != CurrentTime)
    return time;
...
static ClutterActor *
get_key_focused_actor (LemonGlobal *global)
{
  ClutterActor *actor;
...
static void
sync_input_region (LemonGlobal *global)
{
...
/**
 * lemon_global_begin_modal:
 * @global: a #LemonGlobal
 *
 * Grabs the keyboard and mouse to the stage window. The stage will
 * receive all keyboard and mouse events until lemon_global_end_modal()
 ...
 *  window or alt-Tab window selection, or because lemon_global_begin_modal()
 *  was previously called.
 */
gboolean
lemon_global_begin_modal (LemonGlobal       *global,
                          guint32           timestamp,
...
/**
 * lemon_global_end_modal:
 * @global: a #LemonGlobal
 *
 * Undoes the effect of lemon_global_begin_modal().
 */
void
lemon_global_end_modal (LemonGlobal *global,
                        guint32      timestamp)
{
  if (!meta_display_get_compositor (global->meta_display))
    return;
...
/**
 * lemon_global_reexec_self:
 * @global: A #LemonGlobal
 *
 * Initiates the shutdown sequence.
 */
void
lemon_global_reexec_self (LemonGlobal *global)
{
  meta_restart ("Restarting Lemon...");
}

/**
 * lemon_global_real_restart:
 * @global: A #LemonGlobal
 *
 * Restart the current process.
 */
void 
lemon_global_real_restart (LemonGlobal *global)
{
  GPtrArray *arr;
  gsize len;
  ...
  meta_display_close (global->meta_display,
                      lemon_global_get_current_time (global));
 ...
 */
void
lemon_global_notify_error (LemonGlobal  *global,
                           const char   *msg,
                           const char   *details)
{
 ...
 */
void
lemon_global_get_pointer (LemonGlobal         *global,
                          int                 *x,
                          int                 *y,
 ...
 */
void
lemon_global_set_pointer (LemonGlobal         *global,
                          int                 x,
                          int                 y)
{
  ClutterSeat *seat;
 ...
 */
void
lemon_global_sync_pointer (LemonGlobal *global)
{
  int x, y;
  ClutterModifierType mods;
  ClutterEvent *event;
  ...
  lemon_global_get_pointer (global, &x, &y, &mods);

  seat = clutter_backend_get_default_seat (clutter_get_default_backend ());
  event = clutter_event_new (CLUTTER_MOTION);

  event->motion.time = lemon_global_get_current_time (global);
 ...
 */
GSettings *
lemon_global_get_settings (LemonGlobal *global)
{
  return global->settings;
}

/**
 * lemon_global_get_current_time:
 * @global: A #LemonGlobal
 *
 ...
 */
guint32
lemon_global_get_current_time (LemonGlobal *global)
{
  guint32 time;
...
/**
 * lemon_global_get_pid:
 *
 * Return value: the pid of the lemon process.
 */
pid_t
lemon_global_get_pid (LemonGlobal *global)
{
  return getpid();
}

/**
 * lemon_global_get_md5_for_string:
 * @string: input string
 *
 * Return value: (transfer full): the MD5 sum for the given string
 */
gchar *
lemon_global_get_md5_for_string (LemonGlobal *global, const gchar *string)
{
    return g_compute_checksum_for_string (G_CHECKSUM_MD5, string, -1);
}

static void
lemon_global_app_launched_cb (GAppLaunchContext *context,
                                 GAppInfo          *info,
...
/**
 * lemon_global_create_app_launch_context:
...
lemon_global_create_app_launch_context (LemonGlobal *global)
{
  ...
  meta_launch_context_set_timestamp (context, lemon_global_get_current_time (global));
  ...
  g_signal_connect (context,
                    "launched",
                    G_CALLBACK (lemon_global_app_launched_cb),
                    NULL);
...
static void
schedule_leisure_functions (LemonGlobal *global)
{
...
/**
 * lemon_global_begin_work:
 * @global: the #LemonGlobal
 *
 * Marks that we are currently doing work. This is used to to track
 * whether we are busy for the purposes of lemon_global_run_at_leisure().
 * A count is kept and lemon_global_end_work() must be called exactly
 * as many times as lemon_global_begin_work().
 */
void
lemon_global_begin_work (LemonGlobal *global)
{
  global->work_count++;
}

/**
 * lemon_global_end_work:
 * @global: the #LemonGlobal
 *
 * Marks the end of work that we started with lemon_global_begin_work().
 * If no other work is ongoing and functions have been added with
 * lemon_global_run_at_leisure(), they will be run at the next
 * opportunity.
 */
void
lemon_global_end_work (LemonGlobal *global)
{
...
/**
 * lemon_global_run_at_leisure:
 * @global: the #LemonGlobal
 * @func: function to call at leisure
 * @user_data: data to pass to @func
 * @notify: function to call to free @user_data
 *
 * Schedules a function to be called the next time Lemon is idle.
 * Idle means here no animations, no redrawing, and no ongoing background
 * work. Since there is currently no way to hook into the Clutter master
 * clock and know when is running, the implementation here is somewhat
 * approximation. Animations done through Lemon's Tweener module will
 ...
 */
void
lemon_global_run_at_leisure (LemonGlobal         *global,
                             LemonLeisureFunction func,
                             gpointer             user_data,
                             GDestroyNotify       notify)
{
...
/**
 * lemon_global_segfault:
 * @global: the #LemonGlobal
 *
 * Crashes Lemon by causing a segfault
 */
void
lemon_global_segfault (LemonGlobal *global)
{
  int *ptr = NULL;
 ...
 */
void
lemon_global_alloc_leak (LemonGlobal *global, gint mb)
{
    gint i;
...
/**
 * lemon_global_get_stage_xwindow:
 * @global: A #LemonGlobal
 *
 * Returns the X11 window ID of the stage window backing the compositor.
 * This can be used to monitor lemon's liveness from external processes.
 *
 * Returns: The X11 Window ID, or 0 if not available
 */
gulong
lemon_global_get_stage_xwindow (LemonGlobal *global)
{
    g_return_val_if_fail (LEMON_IS_GLOBAL (global), 0);
