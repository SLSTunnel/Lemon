...
#include "lemon-keyring-prompt.h"
#include "lemon-secure-text-buffer.h"
...
typedef struct _LemonPasswordPromptPrivate  LemonPasswordPromptPrivate;
...
static void lemon_keyring_prompt_iface (GcrPromptIface *iface);

G_DEFINE_TYPE_WITH_CODE (LemonKeyringPrompt, lemon_keyring_prompt, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GCR_TYPE_PROMPT, lemon_keyring_prompt_iface);
);
...
static void
lemon_keyring_prompt_init (LemonKeyringPrompt *self)
{
...
static void
lemon_keyring_prompt_set_property (GObject      *obj,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (obj);
  ...
  case PROP_PASSWORD_ACTOR:
    lemon_keyring_prompt_set_password_actor (self, g_value_get_object (value));
    break;
  case PROP_CONFIRM_ACTOR:
    lemon_keyring_prompt_set_confirm_actor (self, g_value_get_object (value));
    break;
  default:
...
static void
lemon_keyring_prompt_get_property (GObject    *obj,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  ...
  case PROP_CHOICE_VISIBLE:
    ...
    break;
  case PROP_PASSWORD_ACTOR:
    g_value_set_object (value, lemon_keyring_prompt_get_password_actor (self));
    break;
  case PROP_CONFIRM_ACTOR:
    g_value_set_object (value, lemon_keyring_prompt_get_confirm_actor (self));
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID (obj, prop_id, pspec);
    break;
...
static void
lemon_keyring_prompt_dispose (GObject *obj)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (obj);
  ...
  if (self->task)
    lemon_keyring_prompt_cancel (self);
  g_assert (self->task == NULL);

  lemon_keyring_prompt_set_password_actor (self, NULL);
  lemon_keyring_prompt_set_confirm_actor (self, NULL);

  G_OBJECT_CLASS (lemon_keyring_prompt_parent_class)->dispose (obj);
}

static void
lemon_keyring_prompt_finalize (GObject *obj)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (obj);
  ...
  G_OBJECT_CLASS (lemon_keyring_prompt_parent_class)->finalize (obj);
}

static void
lemon_keyring_prompt_class_init (LemonKeyringPromptClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = lemon_keyring_prompt_get_property;
  gobject_class->set_property = lemon_keyring_prompt_set_property;
  gobject_class->dispose = lemon_keyring_prompt_dispose;
  gobject_class->finalize = lemon_keyring_prompt_finalize;
  ...
  /**
   * LemonKeyringPrompt:password-visible:
   *
  ...
  /**
    * LemonKeyringPrompt:confirm-visible:
    *
  ...
  /**
   * LemonKeyringPrompt:warning-visible:
   *
  ...
  /**
   * LemonKeyringPrompt:choice-visible:
   *
  ...
  /**
   * LemonKeyringPrompt:password-actor:
   *
  ...
  /**
   * LemonKeyringPrompt:confirm-actor:
   *
...
static void
lemon_keyring_prompt_password_async (GcrPrompt          *prompt,
                                        GCancellable       *cancellable,
                                        GAsyncReadyCallback callback,
                                        gpointer            user_data)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (prompt);
  GObject *obj;
  ...
  g_task_set_source_tag (self->task, lemon_keyring_prompt_password_async);
...
static const gchar *
lemon_keyring_prompt_password_finish (GcrPrompt    *prompt,
                                         GAsyncResult *result,
                                         GError      **error)
{
  g_return_val_if_fail (g_task_get_source_object (G_TASK (result)) == prompt, NULL);
  g_return_val_if_fail (g_async_result_is_tagged (result,
                        lemon_keyring_prompt_password_async), NULL);

  return g_task_propagate_pointer (G_TASK (result), error);
}

static void
lemon_keyring_prompt_confirm_async (GcrPrompt          *prompt,
                                       GCancellable       *cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer            user_data)
{
  ...
  self->task = g_task_new (self, NULL, callback, user_data);
  g_task_set_source_tag (self->task, lemon_keyring_prompt_confirm_async);
...
lemon_keyring_prompt_confirm_finish (GcrPrompt    *prompt,
                                        GAsyncResult *result,
                                        GError      **error)
{
  ...
  g_return_val_if_fail (g_async_result_is_tagged (result,
                        lemon_keyring_prompt_confirm_async), GCR_PROMPT_REPLY_CANCEL);
...
static void
lemon_keyring_prompt_close (GcrPrompt *prompt)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (prompt);
...
static void
lemon_keyring_prompt_iface (GcrPromptIface *iface)
{
  iface->prompt_password_async = lemon_keyring_prompt_password_async;
  iface->prompt_password_finish = lemon_keyring_prompt_password_finish;
  iface->prompt_confirm_async = lemon_keyring_prompt_confirm_async;
  iface->prompt_confirm_finish = lemon_keyring_prompt_confirm_finish;
  iface->prompt_close = lemon_keyring_prompt_close;
}

/**
 * lemon_keyring_prompt_new:
 *
 ...
 */
LemonKeyringPrompt *
lemon_keyring_prompt_new (void)
{
    return g_object_new (LEMON_TYPE_KEYRING_PROMPT, NULL);
}

/**
 * lemon_keyring_prompt_get_password_actor:
 ...
 */
ClutterText *
lemon_keyring_prompt_get_password_actor (LemonKeyringPrompt *self)
{
  g_return_val_if_fail (LEMON_IS_KEYRING_PROMPT (self), NULL);
  return self->password_actor;
}

/**
 * lemon_keyring_prompt_get_confirm_actor:
 * @self: the internal prompt
 *
 * Get the prompt password text actor
 *
 ...
 */
ClutterText *
lemon_keyring_prompt_get_confirm_actor (LemonKeyringPrompt *self)
{
  g_return_val_if_fail (LEMON_IS_KEYRING_PROMPT (self), NULL);
...
on_password_changed (ClutterText *text,
                     gpointer     user_data)
{
  LemonKeyringPrompt *self = LEMON_KEYRING_PROMPT (user_data);
...
/**
 * lemon_keyring_prompt_set_password_actor:
 * @self: the internal prompt
 ...
 */
void
lemon_keyring_prompt_set_password_actor (LemonKeyringPrompt *self,
                                            ClutterText           *password_actor)
{
  ClutterTextBuffer *buffer;

  g_return_if_fail (LEMON_IS_KEYRING_PROMPT (self));
  ...
  if (password_actor)
    {
      buffer = lemon_secure_text_buffer_new ();
...
/**
 * lemon_keyring_prompt_set_confirm_actor:
 * @self: the internal prompt
 ...
 */
void
lemon_keyring_prompt_set_confirm_actor (LemonKeyringPrompt *self,
                                           ClutterText           *confirm_actor)
{
  ClutterTextBuffer *buffer;
 ...
 */
gboolean
lemon_keyring_prompt_complete (LemonKeyringPrompt *self)
{
  GTask *res;
  PromptingMode mode;
  const gchar *password;
  ...
  g_return_val_if_fail (LEMON_IS_KEYRING_PROMPT (self), FALSE);
...
/**
 * lemon_keyring_prompt_cancel:
 * @self: the internal prompt
 *
 * Called by implementation when the prompt is cancelled.
 */
void
lemon_keyring_prompt_cancel (LemonKeyringPrompt *self)
{
  GTask *res;
  PromptingMode mode;
