...
#include "lemon-recorder-src.h"
#include "lemon-recorder.h"
...
struct _LemonRecorderClass
{
  GObjectClass parent_class;
};

struct _LemonRecorder {
  GObject parent;
...
static void recorder_set_stage    (LemonRecorder *recorder,
                                   ClutterStage  *stage);
static void recorder_set_framerate (LemonRecorder *recorder,
                                    int framerate);
static void recorder_set_pipeline (LemonRecorder *recorder,
                                   const char    *pipeline);
static void recorder_set_file_template (LemonRecorder *recorder,
                                        const char    *file_template);
static void recorder_set_draw_cursor (LemonRecorder *recorder,
                                      gboolean       draw_cursor);
...
static void recorder_remove_redraw_timeout (LemonRecorder *recorder);
...
G_DEFINE_TYPE(LemonRecorder, lemon_recorder, G_TYPE_OBJECT);
...
static void
lemon_recorder_init (LemonRecorder *recorder)
{
  /* Calling gst_init() is a no-op if GStreamer was previously initialized */
  gst_init (NULL, NULL);

  lemon_recorder_src_register ();
...
static void
lemon_recorder_finalize (GObject  *object)
{
  LemonRecorder *recorder = LEMON_RECORDER (object);
  ...
  G_OBJECT_CLASS (lemon_recorder_parent_class)->finalize (object);
}
 ...
 */
static void
recorder_update_memory_used (LemonRecorder *recorder,
                             gboolean       repaint)
{
  guint memory_used = 0;
  GSList *l;
...
static void
recorder_add_redraw_timeout (LemonRecorder *recorder)
{
...
static void
recorder_remove_redraw_timeout (LemonRecorder *recorder)
{
  if (recorder->redraw_timeout != 0)
    {
...
static void
recorder_fetch_cursor_image (LemonRecorder *recorder)
{
  CoglTexture *texture;
  int width, height;
  int stride;
 ...
 */
static void
recorder_draw_cursor (LemonRecorder *recorder,
                      GstBuffer     *buffer)
{
  GstMapInfo info;
 ...
 */
static void
recorder_record_frame (LemonRecorder *recorder,
                       gboolean          paint)
{
  GstBuffer *buffer;
  ...
  * indicator from flashing between red and yellow. */
  ...
  if (n_captures == 1)
    ...
    image = lemon_util_composite_capture_images (captures,
                                                 n_captures,
                                                 recorder->area.x,
                                                 recorder->area.y,
  ...
  lemon_recorder_src_add_buffer (LEMON_RECORDER_SRC (recorder->current_pipeline->src), buffer);
...
static void
recorder_update_size (LemonRecorder *recorder)
{
...
static void
recorder_queue_redraw (LemonRecorder *recorder)
{
...
static void
recorder_update_pointer (LemonRecorder *recorder)
{
...
static void
recorder_add_update_pointer_timeout (LemonRecorder *recorder)
{
...
static void
recorder_remove_update_pointer_timeout (LemonRecorder *recorder)
{
  if (recorder->update_pointer_timeout)
    {
...
static void
recorder_connect_stage_callbacks (LemonRecorder *recorder)
{
...
static void
recorder_disconnect_stage_callbacks (LemonRecorder *recorder)
{
...
static void
recorder_set_stage (LemonRecorder *recorder,
                    ClutterStage  *stage)
{
...
static void
recorder_set_display (LemonRecorder *recorder,
                      MetaDisplay   *display)
{
...
static void
recorder_set_framerate (LemonRecorder *recorder,
                        int framerate)
{
...
static void
recorder_set_pipeline (LemonRecorder *recorder,
                       const char    *pipeline)
{
...
static void
recorder_set_file_template (LemonRecorder *recorder,
                            const char    *file_template)
{
...
static void
recorder_set_draw_cursor (LemonRecorder *recorder,
                          gboolean       draw_cursor)
{
...
static void
lemon_recorder_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
lemon_recorder_get_property (GObject         *object,
                             guint            prop_id,
                             GValue          *value,
                             GParamSpec      *pspec)
{
...
static void
lemon_recorder_class_init (LemonRecorderClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = lemon_recorder_finalize;
  gobject_class->get_property = lemon_recorder_get_property;
  gobject_class->set_property = lemon_recorder_set_property;
...
{
  LemonRecorder *recorder = pipeline->recorder;
  GstCaps *caps;
 ...
 * LemonRecorderSrc element where we inject frames then additional elements
...
recorder_pipeline_add_source (RecorderPipeline *pipeline)
{
  ...
  if (sink_pad == NULL)
    {
      g_warning("LemonRecorder: pipeline has no unlinked sink pad");
      goto out;
    }

  pipeline->src = gst_element_factory_make ("lemonrecordersrc", NULL);
  ...
  if (!src_pad)
    {
      g_warning("LemonRecorder: can't get src pad to link into pipeline");
      goto out;
    }

  if (gst_pad_link (src_pad, sink_pad) != GST_PAD_LINK_OK)
    {
      g_warning("LemonRecorder: can't link to sink pad");
      goto out;
    }
 ...
 */
static int
recorder_open_outfile (LemonRecorder  *recorder,
                       char          **outfilename)
{
  const char *pattern;
  char *date_str, *path;
  ...
  if (date_str == NULL)
    {
      date_str = g_strdup_printf ("lemon-%u", g_random_int ());
      g_warning ("Invalid filename template provided to LemonRecorder. Filename will be %s", date_str);
    }
...
recorder_pipeline_add_sink (RecorderPipeline *pipeline)
{
  ...
  if (!sink_pad)
    {
      g_warning("LemonRecorder: can't get sink pad to link pipeline output");
      goto out;
    }
 ...
 */
static void
recorder_pipeline_on_memory_used_changed (LemonRecorderSrc *src,
                                          GParamSpec       *spec,
...
static gboolean
recorder_open_pipeline (LemonRecorder *recorder)
{
  RecorderPipeline *pipeline;
  ...
  if (pipeline->pipeline == NULL)
    {
      g_warning ("LemonRecorder: failed to parse pipeline: %s", error->message);
      g_error_free (error);
      goto error;
    }
...
static void
recorder_close_pipeline (LemonRecorder *recorder)
{
 ...
 *
 * Create a new #LemonRecorder to record movies of a #ClutterStage
 *
 * Return value: The newly created #LemonRecorder object
 */
LemonRecorder     *
lemon_recorder_new (ClutterStage  *stage)
{
  return g_object_new (LEMON_TYPE_RECORDER,
                       "stage",    stage,
                       NULL);
}

/**
 * lemon_recorder_set_framerate:
 * @recorder: the #LemonRecorder
 ...
 */
void
lemon_recorder_set_framerate (LemonRecorder *recorder,
                             int framerate)
{
  g_return_if_fail (LEMON_IS_RECORDER (recorder));

  recorder_set_framerate (recorder, framerate);
}

/**
 * lemon_recorder_set_file_template:
 * @recorder: the #LemonRecorder
 ...
 * %%: A literal percent
 *
 * The default value is 'lemon-%d%u-%c.ogg'.
 */
void
lemon_recorder_set_file_template (LemonRecorder *recorder,
                                  const char    *file_template)
{
  g_return_if_fail (LEMON_IS_RECORDER (recorder));
...
void
lemon_recorder_set_draw_cursor (LemonRecorder *recorder,
                                gboolean       draw_cursor)
{
  g_return_if_fail (LEMON_IS_RECORDER (recorder));
 ...
 * output file. (See lemon_recorder_set_file_template().) However
 ...
 */
void
lemon_recorder_set_pipeline (LemonRecorder *recorder,
                             const char    *pipeline)
{
  g_return_if_fail (LEMON_IS_RECORDER (recorder));
...
void
lemon_recorder_set_area (LemonRecorder *recorder,
                            int               x,
                            int               y,
                            int               width,
                            int               height)
 ...
 */
gboolean
lemon_recorder_record (LemonRecorder  *recorder,
                       char          **filename_used)
{
  g_return_val_if_fail (LEMON_IS_RECORDER (recorder), FALSE);
  ...
  meta_disable_unredirect_for_display (lemon_global_get_display (lemon_global_get ()));
...
/**
 * lemon_recorder_close:
 * @recorder: the #LemonRecorder
 *
 * Stops recording. It's possible to call lemon_recorder_record()
 ...
 */
void
lemon_recorder_close (LemonRecorder *recorder)
{
  g_return_if_fail (LEMON_IS_RECORDER (recorder));
  ...
  meta_enable_unredirect_for_display (lemon_global_get_display (lemon_global_get ()));
 ...
 */
gboolean
lemon_recorder_is_recording (LemonRecorder *recorder)
{
  g_return_val_if_fail (LEMON_IS_RECORDER (recorder), FALSE);
