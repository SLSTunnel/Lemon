...
sys.path.insert(0, '/usr/share/lemon/lemon-menu-editor')
from cme import util

sys.path.insert(0, '/usr/share/lemon/lemon-settings')
...
PANEL_LAUNCHER_PATH = os.path.join(GLib.get_user_data_dir(), "lemon", "panel-launchers")
OLD_PANEL_LAUNCHER_PATH = os.path.join(home, ".lemon", "panel-launchers")

EXTENSIONS = (".png", ".xpm", ".svg")

DEFAULT_ICON_NAME = "lemon-panel-launcher"
...
class ItemEditor(object):
    ...
    def __init__(self, item_path=None, callback=None, destdir=None, icon_size=24):
        ...
        self.builder.set_translation_domain('lemon') # let it translate!
...
class LauncherEditor(ItemEditor):
    ui_file = '/usr/share/lemon/lemon-desktop-editor/launcher-editor.ui'
...
class DirectoryEditor(ItemEditor):
    ui_file = '/usr/share/lemon/lemon-desktop-editor/directory-editor.ui'
...
class LemonLauncherEditor(ItemEditor):
    ui_file = '/usr/share/lemon/lemon-desktop-editor/launcher-editor.ui'
    ...
    def check_custom_path(self):
        ...
        if self.item_path is None or "lemon-custom-launcher" not in self.item_path:
            i = 1
            while True:
                name = os.path.join(PANEL_LAUNCHER_PATH, 'lemon-custom-launcher-' + str(i) + '.desktop')
                old_name = os.path.join(OLD_PANEL_LAUNCHER_PATH, 'lemon-custom-launcher-' + str(i) + '.desktop')
    ...
    def load(self):
        super(LemonLauncherEditor, self).load()
        self.set_text('name-entry', "Name")
        self.set_text('exec-entry', "Exec")
        self.set_text('comment-entry', "Comment")
        self.set_check('terminal-check', "Terminal")
...
class Main:
    def __init__(self):
        ...
        if options.mode == "lemon-launcher" and len(args) < 1:
            parser.error("lemon-launcher mode must have the following syntax:\n"
                         "lemon-desktop-editor -mlemon-launcher [-ffoo.desktop] <json-path>")

        self.tree = CMenu.Tree.new("lemon-applications.menu", CMenu.TreeFlags.INCLUDE_NODISPLAY)
        ...
        if options.mode == "lemon-launcher":
        ...
        if self.mode == "directory":
            ...
            editor.dialog.show_all()
        elif self.mode == "lemon-launcher":
            editor = LemonLauncherEditor(self.orig_file, self.panel_launcher_cb, icon_size=self.icon_size)
            editor.dialog.show_all()
...
if __name__ == "__main__":
    setproctitle("lemon-desktop-editor")
