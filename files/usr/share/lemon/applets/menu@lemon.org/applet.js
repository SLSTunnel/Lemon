        ...
        switch (this._action) {
            case "add_to_panel":
                if (!Main.AppletManager.get_role_provider_exists(Main.AppletManager.Roles.PANEL_LAUNCHER)) {
                    ...
                    enabled_applets.push("panel1:right:0:panel-launchers@lemon.org:" + new_applet_id);
            ...
            case "remove_from_favorites":
                ...
                break;
            case "app_properties":
                Util.spawnCommandLine("lemon-desktop-editor -mlauncher -o" + GLib.shell_quote(this._button.app.get_app_info().get_filename()));
                break;
            case "uninstall":
                Util.spawnCommandLine("/usr/bin/lemon-remove-application '" + this._button.app.get_app_info().get_filename() + "'");
                break;
            case "offload_launch":
                try {
...
class LemonMenuApplet extends Applet.TextIconApplet {
    constructor(orientation, panel_height, instance_id) {
        ...
        edit_item.connect("activate", () => Util.spawnCommandLine("lemon-menu-editor"));
        this._applet_context_menu.addMenuItem(edit_item);

        this.settings = new Settings.AppletSettings(this, "menu@lemon.org", instance_id);
        ...
        this._canUninstallApps = GLib.file_test("/usr/bin/lemon-remove-application", GLib.FileTest.EXISTS);
    ...
    _launch_editor() {
        Util.spawnCommandLine("lemon-menu-editor");
    }
    ...
    _getNewVectorInfo() {
        ...
        if (LemonMenuApplet.DEBUG_VMASK) {
    ...
    _keepMaskActive() {
        ...
        if (Math.abs(Math.hypot(x_dist, y_dist)) < LemonMenuApplet.MIN_MOVEMENT) {
            return false;
        }
    ...
    _enableVectorMask(actor) {
        ...
        this.vector_update_loop = Mainloop.timeout_add(LemonMenuApplet.POLL_INTERVAL, this._maskPollTimeout.bind(this));
    }
    ...
    _refreshApps() {
        ...
        this._allAppsCategoryButton = new CategoryButton(this, null, _("All Applications"), "lemon-all-applications", true);
    ...
    _buildSidebar() {
        ...
        this.userIcon.connect('button-press-event', () => {
            this.menu.toggle();
            Util.spawnCommandLine("lemon-settings user");
        });
    ...
    _display() {
        ...
        this.a11y_settings = new Gio.Settings({ schema_id: "org.lemon.desktop.a11y.applications" });
        this.a11y_settings.connect("changed::screen-magnifier-enabled", this._updateVFade.bind(this));
        this.a11y_mag_settings = new Gio.Settings({ schema_id: "org.lemon.desktop.a11y.magnifier" });
...
function main(metadata, orientation, panel_height, instance_id) {
    return new LemonMenuApplet(orientation, panel_height, instance_id);
}
