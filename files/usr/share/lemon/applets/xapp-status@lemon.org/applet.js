...
class XAppStatusIcon {
    ...
    _onImageLoaded(cache, handle, actor, data=null) {
        if (handle !== this.icon_loader_handle) {
            global.logError(`xapp-status@lemon.org: Icon or image seems out of sync (${this.name}`);
            return;
        }
    ...
    getEventPositionInfo(actor) {
        let allocation = Lemon.util_get_transformed_allocation(actor);
...
class LemonXAppStatusApplet extends Applet.Applet {
...
function main(metadata, orientation, panel_height, instance_id) {
    return new LemonXAppStatusApplet(orientation, panel_height, instance_id);
}
