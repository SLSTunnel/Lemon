...
const DESKTOP_SCHEMA = 'org.lemon.desktop.interface';

// in org.lemon.desktop.interface
...
var Calendar = class Calendar {
    constructor(settings, events_manager) {
        ...
        this._weekStart = Lemon.util_get_week_start();
        this._weekdate = NaN;
    ...
    _onSettingsChange(object, key, old_val, new_val) {
        if (key == FIRST_WEEKDAY_KEY) this._weekStart = Lemon.util_get_week_start();
        this._buildHeader();
        this._update(false);
    }
    ...
    _update(forceReload) {
        ...
        while (true) {
            ...
            let dot_box = new Lemon.GenericContainer(
                {
            ...
            // Hack used in lieu of border-collapse - see lemon.css
            if (row == 2)
