...
var EventsManager = class EventsManager {
    ...
    eds_service_found(connection, name, name_owner) {
        ...
        if (this._calendar_server == null) {
            log("calendar@lemon.org: Calendar events supported.")

            Lemon.CalendarServerProxy.new_for_bus(
                Gio.BusType.SESSION,
                Gio.DBusProxyFlags.DO_NOT_AUTO_START_AT_CONSTRUCTION,
                "org.lemon.CalendarServer",
                "/org/lemon/CalendarServer",
                null,
    ...
    log_dbus_error(e) {
        global.logError(`calendar@lemon.org: Could not check for calendar event support: ${e.toString()}`);
    }

    _calendar_server_ready(obj, res) {
        try {
            this._calendar_server = Lemon.CalendarServerProxy.new_for_bus_finish(res);
    ...
    fetch_month_events(month_year, force) {
        ...
        let week_start = Lemon.util_get_week_start();
