...
class PanelSettingsPage(SettingsPage):
    def __init__(self, panel_id, settings, position):
        ...
        widget = PanelComboBox(_("Auto-hide panel"), "org.lemon", "panels-autohide", self.panel_id, options, size_group=self.size_group)
        section.add_row(widget)

        widget = PanelSpinButton(_("Show delay"), "org.lemon", "panels-show-delay", self.panel_id, _("milliseconds"), 0, 2000, 50, 200)#, dep_key="org.lemon/panels-autohide")
        section.add_reveal_row(widget, "org.lemon", "panels-autohide", check_func=can_show)

        widget = PanelSpinButton(_("Hide delay"), "org.lemon", "panels-hide-delay", self.panel_id, _("milliseconds"), 0, 2000, 50, 200)#, dep_key="org.lemon/panels-autohide")
        section.add_reveal_row(widget, "org.lemon", "panels-autohide", check_func=can_show)

        section = SettingsSection(_("Customize"))
        self.add(section)

        widget = PanelRange(dimension_text, "org.lemon", "panels-height", self.panel_id, _("Smaller"), _("Larger"), mini=20, maxi=60, digits=0, step=1.0, show_value=True)
    ...
    def create_zone_page(self, zone):
        ...
        widget = PanelJSONComboBox(_("Font size"),
                                     "org.lemon", "panel-zone-text-sizes",
        ...
        widget = PanelJSONComboBox(_("Colored icon size"),
                                   "org.lemon", "panel-zone-icon-sizes",
        ...
        widget = PanelJSONSpinButton(_("Symbolic icon size"),
                                     "org.lemon", "panel-zone-symbolic-icon-sizes",
...
class Module:
    name = "panel"
    category = "prefs"
    comment = _("Manage Lemon panel settings")
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            self.settings = Gio.Settings.new("org.lemon")
            ...
            section.add_row(GSettingsSwitch(_("Allow the pointer to pass through the edges of panels"), "org.lemon", "no-adjacent-panel-barriers"))
