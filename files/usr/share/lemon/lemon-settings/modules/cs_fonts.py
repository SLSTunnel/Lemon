...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            widget = GSettingsFontButton(_("Default font"), "org.lemon.desktop.interface", "font-name", size_group=size_group)
            ...
            widget = GSettingsFontButton(_("Window title font"), "org.lemon.desktop.wm.preferences", "titlebar-font", size_group=size_group)
            settings.add_row(widget)
            ...
            size_group = Gtk.SizeGroup.new(Gtk.SizeGroupMode.HORIZONTAL)
            widget = GSettingsSpinButton(_("Text scaling factor"), "org.lemon.desktop.interface", "text-scaling-factor", step=0.1, size_group=size_group)
            settings.add_row(widget)

            widget = GSettingsComboBox(_("Hinting"), "org.lemon.settings-daemon.plugins.xsettings", "hinting", hinting_options, size_group=size_group)
            widget.set_tooltip_text(_("Hinting allows for producing clear, legible text on screen."))
            settings.add_row(widget)

            widget = GSettingsComboBox(_("Antialiasing"), "org.lemon.settings-daemon.plugins.xsettings", "antialiasing", aa_options, size_group=size_group)
            widget.set_tooltip_text(_("Antialiasing makes on screen text smoother and easier to read"))
            settings.add_row(widget)

            widget = GSettingsComboBox(_("RGBA Order"), "org.lemon.settings-daemon.plugins.xsettings", "rgba-order", rgba_options, size_group=size_group)
