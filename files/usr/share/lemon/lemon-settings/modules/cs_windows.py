...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            widget = GSettingsComboBox(_("Buttons layout"), "org.lemon.desktop.wm.preferences", "button-layout", button_options, size_group=size_group)
            ...
            widget = GSettingsComboBox(_("Action on title bar double-click"), "org.lemon.desktop.wm.preferences", "action-double-click-titlebar", action_options, size_group=size_group)
            settings.add_row(widget)

            widget = GSettingsComboBox(_("Action on title bar middle-click"), "org.lemon.desktop.wm.preferences", "action-middle-click-titlebar", action_options, size_group=size_group)
            settings.add_row(widget)

            widget = GSettingsComboBox(_("Action on title bar right-click"), "org.lemon.desktop.wm.preferences", "action-right-click-titlebar", action_options, size_group=size_group)
            settings.add_row(widget)

            scroll_options = [["none", _("Nothing")],["shade", _("Shade and unshade")],["opacity", _("Adjust opacity")]]

            widget = GSettingsComboBox(_("Action on title bar with mouse scroll"), "org.lemon.desktop.wm.preferences", "action-scroll-titlebar", scroll_options, size_group=size_group)
            settings.add_row(widget)

            spin = GSettingsSpinButton(_("Minimum opacity"), "org.lemon.desktop.wm.preferences", "min-window-opacity", _("%"))
            settings.add_reveal_row(spin)

            spin.revealer.settings = Gio.Settings("org.lemon.desktop.wm.preferences")
            ...
            widget = GSettingsComboBox(_("Window focus mode"), "org.lemon.desktop.wm.preferences", "focus-mode", focus_options)
            ...
            widget = GSettingsSwitch(_("Automatically raise focused windows"), "org.lemon.desktop.wm.preferences", "auto-raise")
            settings.add_reveal_row(widget)

            widget.revealer.settings = Gio.Settings("org.lemon.desktop.wm.preferences")
            ...
            widget = GSettingsSwitch(_("Bring windows which require attention to the current workspace"), "org.lemon.muffin", "bring-windows-to-current-workspace")
            settings.add_row(widget)
            ...
            gsettings = widget.get_settings("org.lemon.desktop.wm.preferences")
            ...
            widget = GSettingsSwitch(_("Attach dialog windows to the parent window"), "org.lemon.muffin", "attach-modal-dialogs")
            settings.add_row(widget)
            ...
            widget = GSettingsComboBox(_("Location of newly opened windows"), "org.lemon.muffin", "placement-mode", placement_options, size_group=size_group)
            settings.add_row(widget)

            special_key_options = [["", _("Disabled")], ["<Alt>", "<Alt>"],["<Super>", "<Super>"],["<Control>", "<Control>"]]
            widget = GSettingsComboBox(_("Special key to move and resize windows"), "org.lemon.desktop.wm.preferences", "mouse-button-modifier", special_key_options, size_group=size_group)
            ...
            widget = GSettingsRange(_("Draggable border width"), "org.lemon.muffin", "draggable-border-width", _("Narrower"), _("Wider"),
            ...
            widget = GSettingsComboBox(_("Alt-Tab switcher style"), "org.lemon", "alttab-switcher-style", alttab_styles)
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Display the alt-tab switcher on the primary monitor instead of the active one"), "org.lemon", "alttab-switcher-enforce-primary-monitor")
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Move minimized windows to the end of the alt-tab switcher"), "org.lemon", "alttab-minimized-aware")
            settings.add_row(widget)

            widget = GSettingsSpinButton(_("Delay before displaying the alt-tab switcher"), "org.lemon", "alttab-switcher-delay", units=_("milliseconds"), mini=0, maxi=1000, step=50, page=150)
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Show windows from all workspaces"), "org.lemon", "alttab-switcher-show-all-workspaces")
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Show windows from current monitor"), "org.lemon", "alttab-switcher-show-current-monitor")
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Warp mouse pointer to the new focused window"), "org.lemon", "alttab-switcher-warp-mouse-pointer")
            settings.add_row(widget)
            ...
            switch = GSettingsSwitch(_("Enable window tiling"), "org.lemon.muffin", "edge-tiling")
            settings.add_row(switch)
            switch = GSettingsSwitch(_("Maximize, instead of tile, when dragging a window to the top edge"), "org.lemon.muffin", "tile-maximize")
            settings.add_reveal_row(switch, "org.lemon.muffin", "edge-tiling")
