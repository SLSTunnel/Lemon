...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            settings.add_row(GSettingsSwitch(_("Use 24h clock"), "org.lemon.desktop.interface", "clock-use-24h"))
            settings.add_row(GSettingsSwitch(_("Display the date"), "org.lemon.desktop.interface", "clock-show-date"))
            settings.add_row(GSettingsSwitch(_("Display seconds"), "org.lemon.desktop.interface", "clock-show-seconds"))
            days = [[7, _("Use locale default")], [0, _("Sunday")], [1, _("Monday")]]
            settings.add_row(GSettingsComboBox(_("First day of week"), "org.lemon.desktop.interface", "first-day-of-week", days, valtype=int))
    ...
    def sync_24h_to_gnome(self):
        ...
        self.our_settings = Gio.Settings(schema_id="org.lemon.desktop.interface")
...
class CsdDBusProxyHandler(object):
    def __init__(self, proxy_ready_callback):
        ...
        try:
            Gio.DBusProxy.new_for_bus(Gio.BusType.SYSTEM, Gio.DBusProxyFlags.NONE, None,
                                      'org.lemon.SettingsDaemon.DateTimeMechanism',
                                      '/org/lemon/SettingsDaemon/DateTimeMechanism',
                                      'org.lemon.SettingsDaemon.DateTimeMechanism',
                                      None, self._on_proxy_ready, None)
        except dbus.exceptions.DBusException as e:
