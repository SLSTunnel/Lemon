...
CONFIG_FOLDER = os.path.join(GLib.get_user_config_dir(), 'lemon', 'backgrounds')
OLD_CONFIG_FOLDER = os.path.expanduser("~/.lemon/backgrounds")
...
# version supported by python2 which (at this time) is still used by older versions of Lemon.
...
class ColorsWidget(SettingsWidget):
    def __init__(self, size_group):
        ...
        #gsettings
        self.settings = Gio.Settings("org.lemon.desktop.background")
...
class Module:
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            self._background_schema = Gio.Settings(schema="org.lemon.desktop.background")
            self._slideshow_schema = Gio.Settings(schema="org.lemon.desktop.background.slideshow")
            ...
            widget = GSettingsSwitch(_("Play backgrounds as a slideshow"), "org.lemon.desktop.background.slideshow", "slideshow-enabled")
            settings.add_row(widget)

            widget = GSettingsSpinButton(_("Delay"), "org.lemon.desktop.background.slideshow", "delay", _("minutes"), 1, 1440)
            settings.add_reveal_row(widget, "org.lemon.desktop.background.slideshow", "slideshow-enabled")

            widget = GSettingsSwitch(_("Play images in random order"), "org.lemon.desktop.background.slideshow", "random-order")
            settings.add_reveal_row(widget, "org.lemon.desktop.background.slideshow", "slideshow-enabled")

            widget = GSettingsComboBox(_("Picture aspect"), "org.lemon.desktop.background", "picture-options", BACKGROUND_PICTURE_OPTIONS, size_group=size_group)
            settings.add_row(widget)
    ...
    def get_system_backgrounds(self):
        picture_list = []
        folder_list = []
        properties_dir = "/usr/share/lemon-background-properties"
        backgrounds = []
        if os.path.exists(properties_dir):
            for i in os.listdir(properties_dir):
                if i.endswith(".xml"):
                    ...
                    # Also, other distros should get equal treatment. If they define lemon-backgrounds and use their own distro name, we should add support for it.
            ...
            try:
                ...
                if os.path.exists(cache_filename):
                    ...
                    try:
                        ...
                        # python2 version of lemon settings. Either way, we want to ditch the current
