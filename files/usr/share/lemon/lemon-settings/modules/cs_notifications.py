...
class Module:
    ...
    def on_module_selected(self):
        ...
        Notify.init("lemon-settings-notifications-test")
        ...
        switch = GSettingsSwitch(_("Enable notifications"), "org.lemon.desktop.notifications", "display-notifications")
        settings.add_row(switch)

        switch = GSettingsSwitch(_("Remove notifications after their timeout is reached"), "org.lemon.desktop.notifications", "remove-old")
        settings.add_reveal_row(switch, "org.lemon.desktop.notifications", "display-notifications")

        switch = GSettingsSwitch(_("Show notifications on the bottom side of the screen"), "org.lemon.desktop.notifications", "bottom-notifications")
        settings.add_reveal_row(switch, "org.lemon.desktop.notifications", "display-notifications")

        combo = GSettingsComboBox(_("Monitor to use for displaying notifications"), "org.lemon.desktop.notifications", "notification-screen-display", NOTIFICATION_DISPLAY_SCREENS)
        settings.add_reveal_row(combo, "org.lemon.desktop.notifications", "display-notifications")

        spin = GSettingsSpinButton(_("Monitor"), "org.lemon.desktop.notifications", "notification-fixed-screen", None, 1, 13, 1)
        settings.add_reveal_row(spin)
        spin.revealer.settings = Gio.Settings("org.lemon.desktop.notifications")
        ...
        switch = GSettingsSwitch(_("Display notifications over fullscreen windows"), "org.lemon.desktop.notifications", "fullscreen-notifications")
        settings.add_reveal_row(switch, "org.lemon.desktop.notifications", "display-notifications")

        spin = GSettingsSpinButton(_("Notification duration"), "org.lemon.desktop.notifications", "notification-duration", _("seconds"), 1, 60, 1, 1)
        settings.add_reveal_row(spin, "org.lemon.desktop.notifications", "display-notifications")

        button = Button(_("Display a test notification"), self.send_test)
        settings.add_reveal_row(button, "org.lemon.desktop.notifications", "display-notifications")

        settings = page.add_section(_("Media keys OSD"))

        switch = GSettingsSwitch(_("Show media keys OSD"), "org.lemon", "show-media-keys-osd")
        settings.add_row(switch)
