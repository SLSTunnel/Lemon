...
from bin.LemonGtkSettings import CssRange, CssOverrideSwitch, GtkSettingsSwitch, PreviewWidget, Gtk2ScrollbarSizeEditor
...
# Gtk and Lemon check folders in order of precedence.  These lists match the
...
THEMES_BLACKLIST = [
    ...
    "hicolor", # same
    "adwaita", "adwaita-dark", "adwaitalegacy", # incomplete outside of GNOME, doesn't support Lemon.
...
class Module:
    ...
    def refresh_themes(self):
        ...
        # Lemon themes
        for (name, path) in walk_directories(THEME_FOLDERS, lambda d: os.path.exists(os.path.join(d, "lemon")), return_directories=True):
            for theme in self.lemon_themes:
                if name == theme[0]:
                    if path == THEME_FOLDERS[0]:
                        continue
                    else:
                        self.lemon_themes.remove(theme)
            self.lemon_theme_names.add(name)
            self.lemon_themes.append((name, path))
        self.lemon_themes.sort(key=lambda a: a[0].lower())
    ...
    def on_module_selected(self):
        if not self.loaded:
            ...
            self.settings = Gio.Settings.new("org.lemon.desktop.interface")
            self.lemon_settings = Gio.Settings.new("org.lemon.theme")
            ...
            self.lemon_chooser = self.create_button_chooser(self.lemon_settings, 'name', 'themes', 'lemon', button_picture_width=125, menu_picture_width=125*self.scale, num_cols=4, frame=True)

            selected_meta_theme = None

            gladefile = "/usr/share/lemon/lemon-settings/themes.ui"
            builder = Gtk.Builder()
            builder.set_translation_domain('lemon')
            ...
            for mode in ["mixed", "dark", "light"]:
                path = f"/usr/share/lemon/lemon-settings/appearance-{mode}.svg"
            ...
            self.color_dot_svg = ""
            with open("/usr/share/lemon/lemon-settings/color_dot.svg") as f:
            ...
            widget = self.make_group(_("Desktop"), self.lemon_chooser)
            settings.add_row(widget)
            ...
            widget = GSettingsSwitch(_("Show icons in menus"), "org.lemon.settings-daemon.plugins.xsettings", "menus-have-icons")
            settings.add_row(widget)

            widget = GSettingsSwitch(_("Show icons on buttons"), "org.lemon.settings-daemon.plugins.xsettings", "buttons-have-icons")
            settings.add_row(widget)
            ...
            widget = GSettingsSwitch(_("Use overlay scroll bars"), "org.lemon.desktop.interface", "gtk-overlay-scrollbars")
            settings.add_row(widget)
...
"""Changes may not apply to already-running programs, and may not affect all applications."""))
    ...
    def is_variant_active(self, variant):
        ...
        if variant.icon_theme != self.settings.get_string("icon-theme"):
            return False
        if variant.lemon_theme != self.lemon_settings.get_string("name"):
            return False
    ...
    def is_variant_valid(self, variant):
        ...
        if variant.icon_theme is None:
            ...
            return False
        if variant.lemon_theme is None:
            print("No Lemon theme defined")
            return False
        ...
        if variant.icon_theme not in self.icon_theme_names:
            ...
            return False
        if variant.lemon_theme not in self.lemon_theme_names and variant.lemon_theme != "lemon":
            print("Lemon theme not found:", variant.lemon_theme)
            return False
    ...
    def on_customize_button_clicked(self, widget):
        ...
        self.set_button_chooser(self.lemon_chooser, self.lemon_settings.get_string("name"), 'themes', 'lemon', 60)
    ...
    def activate_variant(self, variant):
        ...
        self.lemon_settings.set_string("name", variant.lemon_theme)
    ...
    def refresh_choosers(self):
        array = [(self.cursor_chooser, "cursors", self.cursor_themes, self._on_cursor_theme_selected),
                    ...
                    (self.lemon_chooser, "lemon", self.lemon_themes, self._on_lemon_theme_selected),
    ...
    def refresh_chooser(self, chooser, path_suffix, themes, callback):
        ...
        if path_suffix == 'icons':
            ...
            if os.path.exists(icon_cache_path):
                ...
                read_path = '/usr/share/lemon/lemon-settings/icons'
            ...
            if path_suffix == "lemon":
                chooser.add_picture("/usr/share/lemon/theme/thumbnail.png", callback, title="lemon", id="lemon")
            if path_suffix in ["gtk-3.0", "lemon"]:
            ...
            for theme_data in themes: # theme_data is a tuple (name, path)
                name = theme_data[0]
                path = theme_data[1]
                theme_type = 'gtk' if path_suffix == 'gtk-3.0' else 'lemon'
                ...
                for theme_info in single_item_category_themes:
                    ...
                    try:
                        for path_option in [f"{theme_path_val}/{theme_name}/{path_suffix}/thumbnail.png",
                                     f"/usr/share/lemon/thumbnails/{path_suffix}/{theme_name}.png",
                                     f"/usr/share/lemon/thumbnails/{path_suffix}/unknown.png"]:
                            if os.path.exists(path_option):
                                chooser.add_picture(path_option, callback, title=theme_name, id=theme_name)
                                break
                    except:
                        chooser.add_picture(f"/usr/share/lemon/thumbnails/{path_suffix}/unknown.png", callback, title=theme_name, id=theme_name)
                    GLib.timeout_add(5, self.increment_progress, (chooser, inc))
    ...
    def set_button_chooser(self, chooser, theme, path_prefix, path_suffix, button_picture_width):
        ...
        if path_suffix == "lemon" and theme == "lemon":
            chooser.set_picture_from_file("/usr/share/lemon/theme/thumbnail.png")
            ...
            try:
                for path in ([os.path.join(datadir, path_prefix, theme, path_suffix, "thumbnail.png") for datadir in GLib.get_system_data_dirs()]
                             ...
                             f"/usr/share/lemon/thumbnails/{path_suffix}/{theme}.png",
                             f"/usr/share/lemon/thumbnails/{path_suffix}/unknown.png"]):
                    if os.path.exists(path):
                        chooser.set_picture_from_file(path)
                        break
            except:
                chooser.set_picture_from_file(f"/usr/share/lemon/thumbnails/{path_suffix}/unknown.png")
    ...
    def _on_lemon_theme_selected(self, path, theme):
        try:
            self.lemon_settings.set_string("name", theme)
            self.set_button_chooser_text(self.lemon_chooser, theme)
        except Exception as detail:
            print(detail)
        return True
