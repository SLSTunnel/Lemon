...
settings_dir = os.path.join(GLib.get_user_config_dir(), 'lemon', 'spices')
old_settings_dir = f'{home}/.lemon/configs/'

URL_SPICES_HOME = "https://lemon-spices.linuxmint.com"
URL_MAP = {
...
class Spice_Harvester(GObject.Object):
    ...
    def __init__(self, collection_type, window=None):
        ...
        self.cache_folder = os.path.join(GLib.get_user_cache_dir(), 'lemon', 'spices', self.collection_type)

        if self.themes:
            self.settings = Gio.Settings.new('org.lemon.theme')
        ...
        if self.themes:
            ...
            self.install_folder = f'{home}/.local/share/lemon/{self.collection_type}s/'
            self.spices_directories = (f'/usr/share/lemon/{self.collection_type}s/', self.install_folder)
    ...
    def _on_proxy_ready(self, obj, result, data=None):
        try:
            ...
            if self._proxy.get_name_owner():
                ...
                print("org.Lemon proxy created, but no owner - is Lemon running?")
        except GLib.Error as e:
            print(f"Could not establish proxy for org.Lemon: {e}")
