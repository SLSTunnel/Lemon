...
def get_lemon_version():
    version_str = subprocess.check_output(['lemon', '--version'], encoding="utf-8").split()[1]
    return [int(part) for part in version_str.split(".")]


def find_extension_subdir(directory):
    largest = [0]
    curr_a = get_lemon_version()
...
class ManageSpicesRow(Gtk.ListBoxRow):
    def __init__(self, extension_type, metadata, size_groups):
        ...
        self.version_supported = self.is_compatible_with_lemon_version()

    def is_compatible_with_lemon_version(self):
        try:
            # Treat "lemon-version" as a list of minimum required versions
            # if any version in there is lower than our Lemon version,
            # then the spice is compatible.
            curr_ver = get_lemon_version()

            for version in self.metadata['lemon-version']:
                ...
                if spice_ver[:2] <= curr_ver:
                    ...
                    return False
            print(f"The {self.uuid} {self.extension_type} is not compatible with this version of Lemon.")
            return False
        except:
            # If lemon-version is not specified or if the version check goes wrong, assume compatibility
            return True
    ...
    def on_scan_complete(self, is_dangerous):
        if is_dangerous:
            if self.extension_type == "applet":
                self.add_status('dangerous', 'xsi-dialog-warning-symbolic', _("This applet contains function calls that could potentially cause Lemon to crash or freeze. If you are experiencing crashes or freezing, please try removing it."))
            elif self.extension_type == "desklet":
                self.add_status('dangerous', 'xsi-dialog-warning-symbolic', _("This desklet contains function calls that could potentially cause Lemon to crash or freeze. If you are experiencing crashes or freezing, please try removing it."))
            elif self.extension_type == "extension":
                self.add_status('dangerous', 'xsi-dialog-warning-symbolic', _("This extension contains function calls that could potentially cause Lemon to crash or freeze. If you are experiencing crashes or freezing, please try removing it."))
...
class ManageSpicesPage(SettingsPage):
    ...
    def enable_extension(self, uuid, name, version_check=True):
        if not version_check:
            show_message(_("Extension %s is not compatible with your version of Lemon.") % uuid, self.window)
            return
    ...
    def restore_to_default(self, *args):
        ...
        if show_prompt(msg, self.window):
            gio_sett = 'org.nemo.plugins' if self.collection_type == 'action' else 'org.lemon'
