...
gi.require_version("LemonDesktop", "3.0")
gi.require_version('IBus', '1.0')
from gi.repository import GLib, Gio, Gtk, GObject, LemonDesktop, IBus
...
class InputSourceSettingsPage(SettingsPage):
    def __init__(self):
        ...
        builder.add_from_file("/usr/share/lemon/lemon-settings/bin/input-sources-list.ui")
        ...
        widget = GSettingsSwitch(
            ...
            "org.lemon.desktop.input-sources", "per-window"
        )
        section.add_row(widget)

        widget = GSettingsSwitch(
            _("Use a country flag, if available, to represent keyboard layouts"),
            "org.lemon.desktop.interface", "keyboard-layout-show-flags"
        )
        section.add_row(widget)
        widget = GSettingsSwitch(
            _("Use the layout name, not the group name, to represent a layout"),
            "org.lemon.desktop.interface", "keyboard-layout-prefer-variant-names"
        )
        section.add_row(widget)
        widget = GSettingsSwitch(
            _("Use upper-case when using text to represent a layout"),
            "org.lemon.desktop.interface", "keyboard-layout-use-upper"
        )
        section.add_row(widget)
        ...
        widget = GSettingsKeybinding(
            ...
            "org.lemon.desktop.keybindings.wm",
            "switch-input-source",
        )
        section.add_row(widget)
        widget = GSettingsKeybinding(
...
class CurrentInputSourcesModel(GObject.Object, Gio.ListModel):
    ...
    def __init__(self):
        ...
        self._proxy = None
        self._sources = []
        self.interface_settings = Gio.Settings(schema_id="org.lemon.desktop.interface")
        self.interface_settings.connect("changed", self.on_interface_settings_changed)
        self.input_source_settings = Gio.Settings(schema_id="org.lemon.desktop.input-sources")

        try:
            Gio.DBusProxy.new_for_bus(Gio.BusType.SESSION, Gio.DBusProxyFlags.NONE, None,
                                      "org.Lemon", "/org/Lemon", "org.Lemon", None, self._on_proxy_ready, None)
        except GLib.Error as e:
            print(e.message)
            self._proxy = None

        self.xkb_info = LemonDesktop.XkbInfo.new_with_extras()
    ...
    def _on_proxy_ready(self, obj, result, data=None):
        try:
            ...
            self._proxy.connect("notify::g-name-owner", self._on_lemon_state_changed)
            self.refresh_input_source_list()
        except GLib.Error as e:
            print(f"Keyboard module could not establish proxy for org.Lemon: {e}")

    def _on_proxy_signal(self, proxy, sender_name, signal_name, parameters):
        if signal_name == "InputSourcesChanged":
            self.refresh_input_source_list()

    def _on_lemon_state_changed(self, proxy, pspec, data=None):
        # If Lemon crashes, this will happen, reload our sources using xkb and ibus.
        # This isn't reliable to detect Lemon restarting, as it will regain a name-owner
        # before org.Lemon is exported, causing GetInputSources to fail.
        #
    ...
    def remove_layout(self, source):
        ...
        # shouldn't happen but here we're not relying on lemon, but dconf.
