...
class CManager:
    def __init__(self):
        self.extension_point = Gio.io_extension_point_register ("lemon-control-center-1")
        self.modules = []
        ...
        for path in paths:
            if not os.path.islink(path):
                path = os.path.join(path, "lemon-control-center-1/panels")
    ...
    def get_c_widget(self, mod_id):
        ...
        if extension is None:
            print(f"Could not load {mod_id} module; is the lemon-control-center package installed?")
            return None
    ...
    def lookup_c_module(self, mod_id):
        extension = self.extension_point.get_extension_by_name(mod_id)
        if extension is None:
            print(f"Could not find {mod_id} module; is the lemon-control-center package installed?")
            return False
        else:
            return True
