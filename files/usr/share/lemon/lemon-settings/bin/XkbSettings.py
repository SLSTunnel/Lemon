...
from gi.repository import Gio, Gtk, LemonDesktop, Pango, GLib
...
class XkbSettingsEditor(SettingsSection):
    # Groups that are hidden from UI (broken or not supported in Lemon)
    ...
    def _sync_to_gsettings(self):
        ...
        # Restart Lemon if grp options changed (required at the moment for muffin
        # to properly register it).
        if grp_changed and not self.is_wayland:
            GLib.timeout_add(400, self._restart_lemon)

    def _restart_lemon(self):
        try:
            subprocess.Popen(['lemon-dbus-command', 'RestartLemon', '1'],
                             stdout=subprocess.DEVNULL)
        except Exception as e:
            print(f"Failed to restart Lemon: {e}")
