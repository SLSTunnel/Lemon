    ...
    def update_label(self):
        if self.settings.get_boolean('clock-use-24h'):
            if self.get_uses_seconds():
                format_code = gettext.dgettext('lemon-desktop', '%R:%S')
            else:
                format_code = gettext.dgettext('lemon-desktop', '%R')
        else:
            if self.get_uses_seconds():
                format_code = gettext.dgettext('lemon-desktop', '%l:%M:%S %p')
            else:
                format_code = gettext.dgettext('lemon-desktop', '%l:%M %p')
