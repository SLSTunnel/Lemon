...
# i18n
gettext.install("lemon", "/usr/share/locale", names=["ngettext"])
...
STANDALONE_MODULES = [
    ...
    [_("Driver Manager"),             "lemon-driver-manager",              "cs-drivers",                "admin",      _("video, driver, wifi, card, hardware, proprietary, nvidia, radeon, nouveau, fglrx")],
    ...
    [_("Users and Groups"),           "lemon-settings-users",              "cs-user-accounts",          "admin",      _("user, users, account, accounts, group, groups, password")],
...
class MainWindow(Gio.Application):
    ...
    def __init__(self, parsed_args):
        Gio.Application.__init__(self,
                                 application_id=f"org.lemon.Settings_{os.getpid()}",
                                 flags=Gio.ApplicationFlags.NON_UNIQUE | Gio.ApplicationFlags.HANDLES_OPEN)
        self.builder = Gtk.Builder()
        self.builder.set_translation_domain('lemon')  # let it translate!
        self.builder.add_from_file(os.path.join(CURRENT_PATH, "lemon-settings.ui"))
    ...
    def load_sidepage_as_standalone(self, args) -> bool:
        ...
        wm_class = f"lemon-settings {args.module}"
    ...
    def load_ccc_modules(self):
        """Loads all Lemon Control Center settings modules."""
...
To see a list of available tabs for a specific module, use `lemon-settings MODULE --tab help`
...
SORT_TYPE can be specified by number or name as follows:
    ...
    parser = argparse.ArgumentParser(
        description="lemon-settings - Configuration tool for Lemon",
        epilog=EPILOG,
    ...
    if args.module is not None and not find_module_name(args.module):
        if args.module == "user":
            subprocess.Popen(["lemon-settings-user"])
            sys.exit(0)
