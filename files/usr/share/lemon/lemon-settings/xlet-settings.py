...
old_settings_dir = '%s/.lemon/configs/' % home
...
class MainWindow(object):
    def __init__(self, args):
        ...
        self.gsettings = Gio.Settings.new("org.lemon")
        self.custom_modules = {}
    ...
    def load_xlet_data (self):
        self.xlet_dir = f"/usr/share/lemon/{self.type}s/{self.uuid}"
        if not os.path.exists(self.xlet_dir):
            self.xlet_dir = f"{home}/.local/share/lemon/{self.type}s/{self.uuid}"
    ...
    def load_instances(self):
        ...
        old_path = Path(f"{home}/.lemon/configs/{self.uuid}")
        instances = 0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="xlet-settings - Configuration tool for Lemon xlets",
    )
