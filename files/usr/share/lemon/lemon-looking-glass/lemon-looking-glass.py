...
MELANGE_DBUS_NAME = "org.Lemon.Melange"
MELANGE_DBUS_PATH = "/org/Lemon/Melange"
...
class MelangeApp(Gtk.Application):
    def __init__(self):
        Gtk.Application.__init__(self,
                                 application_id="org.Lemon.Melange",
    ...
    def construct_window(self):
        ...
        headerbar.set_subtitle("Lemon Troubleshooting Tool")
        ...
        self.status_label.set_markup(" <span foreground='red'>[ Lemon is OFFLINE! ]</span> ")
        self.status_label.set_tooltip_text("The connection to lemon is broken")
        self.status_label.set_no_show_all(True)
        table.attach(self.status_label, column, column+1, 1, 2, 0, 0, 1)
        column += 1

        box = Gtk.HBox()
        settings = Gio.Settings(schema_id="org.lemon.desktop.keybindings")
    ...
    def create_action_button(self):
        restart_func = lambda junk: os.system("nohup lemon --replace > /dev/null 2>&1 &")
        ...
        menu.append(Gtk.SeparatorMenuItem())
        menu.append(self.create_menu_item('Restart Lemon', restart_func))
        menu.append(self.create_menu_item('Crash Lemon', crash_func))
        menu.append(self.create_menu_item('Reset Lemon Settings', self.on_reset_clicked))
        menu.append(Gtk.SeparatorMenuItem())
    ...
    def on_reset_clicked(self, menu_item):
        dialog = Gtk.MessageDialog(self.window, 0,
                                   ...
                                   "Reset all lemon settings to default?")
        dialog.set_title("Warning: Trying to reset all lemon settings!")

        response = dialog.run()
        dialog.destroy()
        if response == Gtk.ResponseType.YES:
            os.system("gsettings reset-recursively org.lemon &")
...
if __name__ == "__main__":
    setproctitle("lemon-looking-glass")
    app = MelangeApp()
    app.run(sys.argv)
    exit(0)
