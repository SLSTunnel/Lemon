...
SS_SERVICE = "org.lemon.ScreenSaver"
SS_PATH = "/org/lemon/ScreenSaver"
SS_INTERFACE = "org.lemon.ScreenSaver"
...
class ScreensaverCommand:
    ...
    Supports both internal (Main.screenShield) and external (lemon-screensaver) modes.
    """
    def __init__(self, mainloop):
        self.mainloop = mainloop
        self.proxy = None

        parser = argparse.ArgumentParser(description='Lemon Screensaver Command')
        ...
        if args.action_id == Action.VERSION:
            ...
            try:
                ...
                version = version_proxy.get_cached_property('LemonVersion')
                if version:
                    print("lemon-screensaver-command (Lemon %s)" % version.unpack())
                else:
                    print("lemon-screensaver-command (Lemon version unknown)")
            except:
                print("lemon-screensaver-command")
            quit()

        self.action_id = args.action_id
        self.message = args.message

        ss_settings = Gio.Settings.new("org.lemon.desktop.screensaver")
