...
SCHEMAS = "org.lemon.desklets.launcher"
LAUNCHER_KEY = "launcher-list"

HOME_DIR = os.path.expanduser("~")+"/"
CUSTOM_LAUNCHERS_PATH = os.path.join(GLib.get_user_data_dir(), "lemon", "panel-launchers")
OLD_CUSTOM_LAUNCHERS_PATH = HOME_DIR + ".lemon/panel-launchers/"
EDITOR_DIALOG_UI_PATH = "/usr/share/lemon/desklets/launcher@lemon.org/editorDialog.ui"
...
class EditorDialog:
    def __init__(self, desklet_id=-1):
        ...
        if desklet_id != - 1:
            ...
            self.name = launcher
            if self.name[:24] == "lemon-custom-launcher":
                self.launcher_type = "Custom Application"

        self.tree = Gtk.Builder()
        self.tree.set_translation_domain('lemon') # let it translate!
    ...
    def on_edit_ok_clicked(self, widget):
        ...
        if self.desklet_id == -1: # Add new launcher
            settings = Gio.Settings.new("org.lemon")
            ...
            enabled_desklets.append("launcher@lemon.org:%s:0:100" % self.desklet_id)
    ...
    def get_custom_path(self):
        ...
        i = 1
        while True:
            name = 'lemon-custom-launcher-' + str(i) + '.desktop'
